...
IMAGE_NAME="edwinpai-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="edwinpai-net-e2e-$$"
GW_NAME="edwinpai-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "EDWINPAI_GATEWAY_TOKEN=$TOKEN" \
	  -e "EDWINPAI_SKIP_CHANNELS=1" \
	  -e "EDWINPAI_SKIP_GMAIL_WATCHER=1" \
	  -e "EDWINPAI_SKIP_CRON=1" \
	  -e "EDWINPAI_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
