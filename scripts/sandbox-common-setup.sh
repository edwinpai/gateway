...
BASE_IMAGE="${BASE_IMAGE:-edwinpai-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-edwinpai-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=edwinpai.sandbox=1)
NOTE
