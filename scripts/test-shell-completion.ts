 ...
 * `edwinpai update`. Use it to verify the completion installation flow
...
\`edwinpai update\`, \`edwinpai doctor\`, and \`edwinpai onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# EdwinPAI Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`edwinpai completion --install\` later to enable.`));
    return;
  }
