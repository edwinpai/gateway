...
for (const packageDir of packageDirs.sort()) {
  ...
  const tmp = await mkdtemp(path.join(os.tmpdir(), "identity-core-pack-audit-"));
  try {
    ...
    const pack = (
      JSON.parse(raw) as Array<{ filename?: string; files?: Array<{ path?: string }> }>
    )[0];
    if (!pack) throw new Error(`npm pack --dry-run returned no metadata for ${packageJson.name}`);
    const files = (pack.files ?? [])
      .map((file) => file.path)
      .filter((file): file is string => !!file)
      .sort();
    const expected = ["identity-core-artifact.json", "identity-core.node", "package.json"];
