import { mergeAllowlist, summarizeMapping, type RuntimeEnv } from "@edwinpai/edwinpai/plugin-sdk";
...
export async function monitorMatrixProvider(opts: MonitorMatrixOpts = {}): Promise<void> {
  ...
  if (
    auth.encryption &&
    client.crypto &&
    typeof client.crypto.requestOwnUserVerification === "function"
  ) {
    try {
  ...
  } else if (auth.encryption && client.crypto) {
    logger.debug("Matrix SDK does not support requestOwnUserVerification on this runtime");
  }
