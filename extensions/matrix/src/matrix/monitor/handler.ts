...
import {
  ...
  type RuntimeEnv,
} from "@edwinpai/edwinpai/plugin-sdk";
...
export type MatrixMonitorHandlerParams = {
  ...
  core: {
    ...
    channel: (typeof import("@edwinpai/edwinpai/plugin-sdk"))["channel"];
    system: {
      enqueueSystemEvent: (
        text: string,
  ...
  mentionRegexes: ReturnType<
    (typeof import("@edwinpai/edwinpai/plugin-sdk"))["channel"]["mentions"]["buildMentionRegexes"]
  >;
...
export function createMatrixRoomMessageHandler(params: MatrixMonitorHandlerParams) {
  ...
  return async (roomId: string, event: MatrixRawEvent) => {
    try {
              ...
              if (created) {
                ...
                try {
                  await sendMessageMatrix(
                    `room:${roomId}`,
                    [
                      "Edwin: access not configured.",
                      "",
                      `Pairing code: ${code}`,
                      "",
                      "Ask the bot owner to approve with:",
                      "edwin pairing approve matrix <code>",
                    ].join("\n"),
                    { client },
                  );
                } catch (err) {
      ...
      const _contentMsgtype =
        "msgtype" in content ? (content as { msgtype?: string }).msgtype : undefined;
