...
This document describes how the Edwin ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`edwin acp` exposes an ACP agent over stdio and forwards prompts to a running
Edwin Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Edwin Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `edwin acp` over stdio.

Example config:

```bash
edwin config set gateway.remote.url wss://gateway-host:18789
edwin config set gateway.remote.token <token>
```

Example run:

```bash
edwin acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
edwin acp --session agent:main:main
edwin acp --session agent:design:main
edwin acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Edwin ACP” to start a thread.

## Execution Model

- ACP client spawns `edwin acp` and speaks ACP messages over stdio.
...
```bash
edwin acp --session agent:main:main
edwin acp --session-label "support inbox"
edwin acp --reset-session
```
...
`edwin acp` resolves the Gateway URL and auth from CLI flags or config:
