...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `edwinpai-e2e-${name}-`));
  const configDir = path.join(homeDir, ".edwinpai");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          EDWINPAI_SKIP_CHANNELS: "1",
          EDWINPAI_SKIP_BROWSER_CONTROL_SERVER: "1",
          EDWINPAI_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        EDWINPAI_GATEWAY_TOKEN: inst.gatewayToken,
        EDWINPAI_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          EDWINPAI_GATEWAY_PORT: String(gwA.port),
          EDWINPAI_GATEWAY_TOKEN: gwA.gatewayToken,
          EDWINPAI_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          EDWINPAI_GATEWAY_PORT: String(gwB.port),
          EDWINPAI_GATEWAY_TOKEN: gwB.gatewayToken,
          EDWINPAI_GATEWAY_PASSWORD: "",
        }),
