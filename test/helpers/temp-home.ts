...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("EDWINPAI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.EDWINPAI_STATE_DIR = path.join(base, ".edwinpai");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "edwinpai-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".edwinpai", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
