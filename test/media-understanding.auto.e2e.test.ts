...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("edwinpai-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("edwinpai-bin-sherpa-");
      const modelDir = await makeTempDir("edwinpai-sherpa-model-");
      ...
      const cfg: EdwinPAIConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("edwinpai-bin-whispercpp-");
      const modelDir = await makeTempDir("edwinpai-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("edwinpai-bin-gemini-");
      ...
      const cfg: EdwinPAIConfig = { tools: { media: { image: {} } } };
