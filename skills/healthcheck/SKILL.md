---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Edwin deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Edwin cron scheduling for periodic checks, or version status checks on a machine running Edwin (laptop, workstation, Pi, VPS).
---

# Edwin Host Hardening

## Overview

Assess and harden the host running Edwin, then align it to a user-defined risk tolerance without breaking access. Use Edwin security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Edwin changes the host firewall, SSH, or OS updates; it does not.
...
5. Edwin gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Edwin can access sensitive data.
...
### 2) Run Edwin security audits (read-only)

As part of the default read-only checks, run `edwin security audit --deep`. Only offer alternatives if the user requests them:

1. `edwin security audit` (faster, non-probing)
2. `edwin security audit --json` (structured output)

Offer to apply Edwin safe defaults (numbered):

1. `edwin security audit --fix`

Be explicit that `--fix` only tightens Edwin defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Edwin version/update status (read-only)

As part of the default read-only checks, run `edwin update status`.
...
- Credential hygiene notes (location of Edwin creds, prefer disk encryption)
...
After Edwin install or first hardening pass, run at least one baseline audit and version check:

- `edwin security audit`
- `edwin security audit --deep`
- `edwin update status`

Ongoing monitoring is recommended. Use the Edwin cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `edwin cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `edwin update status`
...
Before creating, `edwin cron list` and match on exact `name`. If found, `edwin cron edit <id> ...`.
If not found, `edwin cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `edwin update status` (preferred for source checkouts and channels)
2. `npm view edwin version` (published npm version)

## Edwin command accuracy

Use only supported commands and flags:

- `edwin security audit [--deep] [--fix] [--json]`
- `edwin status` / `edwin status --deep`
- `edwin health --json`
- `edwin update status`
- `edwin cron add|list|runs|run`

Do not invent CLI flags or imply Edwin enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Edwin compaction:
