...
metadata:
  { "edwin": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${EDWINPAI_TMUX_SOCKET_DIR:-${EDWIN_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/edwinpai-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/edwinpai.sock"
SESSION=edwinpai-python
...
- Use `EDWINPAI_TMUX_SOCKET_DIR` (legacy `EDWIN_TMUX_SOCKET_DIR`, `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$EDWINPAI_TMUX_SOCKET_DIR/edwinpai.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `EDWINPAI_TMUX_SOCKET_DIR`).
