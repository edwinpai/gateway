...
export function normalizeServePath(raw?: string): string {
  ...
  // To accept requests at /<path> externally, gws must listen on "/".
  if (base === "/") {
    return "/";
  }
  const withSlash = base.startsWith("/") ? base : `/${base}`;
...
export function buildGwsWatchStartArgs(
...
export function buildGwsWatchServeArgs(cfg: GmailHookRuntimeConfig): string[] {
  const args = [
    "gmail",
    "+watch",
    "--account",
