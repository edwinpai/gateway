...
This directory contains hooks that ship with EdwinPAI. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.edwinpai/workspace`)

**Enable**:

```bash
edwin hooks enable session-memory
```
...
**Output**: `~/.edwinpai/logs/commands.log`

**Enable**:

```bash
edwin hooks enable command-logger
```
...
**Docs**: https://docs.edwinpai.com/hooks/soul-evil

**Enable**:

```bash
edwin hooks enable soul-evil
```
...
homepage: https://docs.edwinpai.com/hooks#my-hook
metadata:
  { "edwin": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.edwinpai/hooks/` (shared across workspaces)
...
```bash
edwin hooks info session-memory
```
...
```bash
edwin hooks enable session-memory
edwin hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.edwinpai/edwinpai.json`:
...
2. Restart gateway: `pkill -9 -f 'edwin.*gateway' && pnpm edwin gateway`
3. Enable the hook: `edwin hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.edwinpai.com/hooks
