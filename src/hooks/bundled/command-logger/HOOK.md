...
homepage: https://docs.edwinpai.com/hooks#command-logger
metadata:
  {
    "edwinpai":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.edwinpai/logs/commands.log`
...
```bash
edwinpai hooks disable command-logger
```
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.edwinpai/logs/commands.log ~/.edwinpai/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/edwinpai`:
   ```
   /home/username/.edwinpai/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.edwinpai/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.edwinpai/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.edwinpai/logs/commands.log | jq .
```
