...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    ...
    const cfg = context.cfg as EdwinPAIConfig | undefined;
    const agentId = resolveAgentIdFromSessionKey(event.sessionKey);
    const workspaceDir = cfg
      ? resolveAgentWorkspaceDir(cfg, agentId)
      : path.join(os.homedir(), ".edwinpai", "workspace");
    ...
    if (sessionFile) {
      ...
      if (sessionContent && cfg) {
        ...
        const edwinpaiRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
        const slugGenPath = path.join(edwinpaiRoot, "llm-slug-generator.js");
