...
import {
  ...
  ensureTopic,
  resolveProjectIdFromGwsCredentials,
  runGcloud,
...
export async function runGmailSetup(opts: GmailSetupOptions) {
  ...
  await ensureDependency("gws", ["@googleworkspace/cli"]);
  ...
  const projectId =
    opts.project ?? parsedTopic?.projectId ?? (await resolveProjectIdFromGwsCredentials());
  // Gmail watch requires the Pub/Sub topic to live in the OAuth client project.
  if (!projectId) {
    throw new Error(
      "GCP project id required (use --project or ensure gws credentials are available)",
    );
  }
  ...
  // Tailscale strips the path before proxying; keep a public path while gws
  ...
  const nextConfig: EdwinPAIConfig = {
    ...baseConfig,
    hooks: {
      ...baseConfig.hooks,
      enabled: true,
  ...
  defaultRuntime.log(`Next: ${formatCliCommand("edwinpai webhooks gmail run")}`);
}
...
export async function runGmailService(opts: GmailRunOptions) {
  ...
  let child = spawnGwsServe(runtimeConfig);
  ...
  child.on("exit", () => {
    ...
    defaultRuntime.log("gws watch serve exited; restarting in 2s");
    setTimeout(() => {
      if (shuttingDown) {
        return;
      }
      child = spawnGwsServe(runtimeConfig);
    }, 2000);
  });
}

function spawnGwsServe(cfg: GmailHookRuntimeConfig) {
  const args = buildGwsWatchServeArgs(cfg);
  defaultRuntime.log(`Starting gws ${args.join(" ")}`);
  return spawn("gws", args, { stdio: "inherit" });
}
