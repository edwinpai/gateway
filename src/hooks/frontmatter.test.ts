...
`;
    ...
    expect(parsed.edwinpai.emoji).toBe("💾");
    expect(parsed.edwinpai.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.edwinpai.emoji).toBe("📝");
    expect(parsed.edwinpai.events).toEqual(["command"]);
    expect(parsed.edwinpai.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.edwinpai.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"edwinpai": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"edwinpai": {"events": ["test"]}}');
  });
...
describe("resolveEdwinPAIMetadata", () => {
  it("extracts edwinpai metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveEdwinPAIMetadata(frontmatter);
  ...
  it("returns undefined when edwinpai key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        edwinpai: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with EdwinPAI" },
            { id: "npm", kind: "npm", package: "@edwinpai/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@edwinpai/hook");
  });
...
homepage: https://docs.edwinpai.com/hooks#session-memory
metadata:
  {
    "edwinpai":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with EdwinPAI" }],
      },
  }
---
...
`;
    ...
    const edwinpai = resolveEdwinPAIMetadata(frontmatter);
    expect(edwinpai).toBeDefined();
    expect(edwinpai?.emoji).toBe("💾");
    expect(edwinpai?.events).toEqual(["command:new"]);
    expect(edwinpai?.requires?.config).toEqual(["workspace.dir"]);
    expect(edwinpai?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const edwinpai = resolveEdwinPAIMetadata(frontmatter);
    expect(edwinpai?.emoji).toBe("disk");
    expect(edwinpai?.events).toEqual(["command:new"]);
  });
});
