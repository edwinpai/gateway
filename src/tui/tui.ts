...
export async function runTui(opts: TuiOptions) {
  ...
  const updateHeader = () => {
    ...
    header.setText(
      theme.header(
        `edwinpai tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`,
      ),
    );
  };
  ...
  const resolveConfiguredModelLabel = (): string | undefined => {
    try {
      const primary = config.agents?.defaults?.model?.primary;
      if (!primary) return undefined;
      // Check if there's a friendly alias configured
      const models = config.agents?.defaults?.models;
      if (models) {
        for (const [key, val] of Object.entries(models)) {
          if (key === primary || key.endsWith(`/${(val as any)?.alias}`)) {
            const alias = (val as any)?.alias;
  ...
  const updateFooter = () => {
    ...
    const sessionLabel = sessionInfo.displayName
      ...
      : sessionKeyLabel;
    const agentLabel = formatAgentLabel(currentAgentId);
    // Prefer the configured primary model for display — the actually-used provider
    // may differ from config when a fallback fires, but the user cares about what
    // they configured, not the implementation detail of which backend ran.
    const configuredPrimary = resolveConfiguredModelLabel();
    const modelLabel =
      configuredPrimary ??
      (sessionInfo.model
