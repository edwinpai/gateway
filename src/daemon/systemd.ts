...
function resolveSystemdServiceName(env: Record<string, string | undefined>): string {
  const override = env.EDWINPAI_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.EDWINPAI_PROFILE);
}
...
export async function installSystemdService({
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: env.EDWINPAI_PROFILE,
      version: environment?.EDWINPAI_SERVICE_VERSION ?? env.EDWINPAI_SERVICE_VERSION,
    });
  ...
  const serviceName = resolveGatewaySystemdServiceName(env.EDWINPAI_PROFILE);
