...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.EDWINPAI_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return envLabel;
  }
  return resolveGatewayLaunchAgentLabel(args?.env?.EDWINPAI_PROFILE);
}
...
export function resolveGatewayLogPaths(env: Record<string, string | undefined>): {
  ...
  const prefix = env.EDWINPAI_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
export async function findLegacyLaunchAgents(
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(effectiveEnv.EDWINPAI_PROFILE)) {
    const plistPath = resolveLaunchAgentPlistPathForLabel(effectiveEnv, label);
    const res = await execLaunchctl(["print", `${domain}/${label}`]);
...
export async function installLaunchAgent({
  ...
  const effectiveEnv = { ...env, ...environment };
  const { logDir, stdoutPath, stderrPath } = resolveGatewayLogPaths(effectiveEnv);
  await fs.mkdir(logDir, { recursive: true });

  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env: effectiveEnv });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(effectiveEnv.EDWINPAI_PROFILE)) {
    const legacyPlistPath = resolveLaunchAgentPlistPathForLabel(effectiveEnv, legacyLabel);
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: effectiveEnv.EDWINPAI_PROFILE,
      version: environment?.EDWINPAI_SERVICE_VERSION ?? effectiveEnv.EDWINPAI_SERVICE_VERSION,
    });
