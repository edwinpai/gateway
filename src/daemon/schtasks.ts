...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.EDWINPAI_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.EDWINPAI_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.EDWINPAI_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.EDWINPAI_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const scriptPath = resolveTaskScriptPath(effectiveEnv);
  ...
  const taskUser = resolveTaskUser(effectiveEnv);
