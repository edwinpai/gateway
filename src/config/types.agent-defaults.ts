...
import type { MemoryRuntimeAttachmentPolicy } from "../memory/public-policy.js";
import type {
...
export type AgentDefaultsConfig = {
  ...
  subagents?: {
    ...
    thinking?: string;
    /** Default named knowledge/memory profile for spawned sub-agents. */
    profile?: string;
    /** Default named knowledge/memory profile keyed by spawned target agent id. */
    profileBindings?: Record<string, string>;
    /** Named knowledge/memory profiles available to spawned sub-agents. */
    profiles?: Record<
      string,
      {
        /** Collection vocabulary aligned with desktop Knowledge.selectedCollections. */
        selectedCollections?: string[];
        /** Optional attachment/mount intent aligned with Knowledge.runtimeAttachmentPolicy. */
        runtimeAttachmentPolicy?: MemoryRuntimeAttachmentPolicy;
      }
    >;
    /** Default memory collections mounted for spawned sub-agents (legacy fallback). */
...
export type AgentCompactionConfig = {
  ...
  /**
   * Proactive auto-compaction: trigger compaction after a successful turn
   * when context usage exceeds this fraction of the model's context window.
   * Range: 0.3–0.9. Default: disabled (undefined).
   * Example: 0.5 = compact when context reaches 50% of window.
   */
