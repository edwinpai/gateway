...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "edwinpai.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.edwinpai/workspace/canvas). */
  root?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/edwinpai"). */
  basePath?: string;
...
/**
 * BSV cryptographic identity authentication configuration.
 * Implements BRC-103 peer-to-peer authentication.
 *
 * @see https://github.com/bitcoin-sv/BRCs/blob/master/peer-to-peer/0103.md
 */
export type GatewayBsvAuthConfig = {
  /**
   * Enable BSV identity authentication for the gateway.
   * When enabled, requests can be authenticated via cryptographic signatures.
   */
  enabled?: boolean;

  /**
   * Enable ECIES encryption for request/response bodies.
   * When true, requests with x-bsv-encrypted: true will have bodies decrypted,
   * and responses to authenticated identities can be encrypted.
   * Default: false.
   */
  enableEncryption?: boolean;

  /**
   * Owner's BSV public key (compressed, 33 bytes hex).
   * Requests signed by this key have owner-level access.
   * Can also be set via BSV_OWNER_PUBLIC_KEY environment variable.
   */
  ownerPublicKey?: string;

  /**
   * Require owner identity for all authenticated requests.
   * When true, only requests from the owner's identity key are allowed.
   * Default: false (any valid BSV identity is allowed).
   */
  ownerOnly?: boolean;

  /**
   * Allow requests without BSV authentication headers.
   * When true, unauthenticated requests pass through with no identity context.
   * When false, requests missing auth headers receive 401 Unauthorized.
   * Default: true (for backwards compatibility with existing auth modes).
   */
  allowUnauthenticated?: boolean;

  /**
   * Paths to skip BSV authentication (e.g., health checks, public endpoints).
   * Supports prefix matching (e.g., "/health" matches "/health/ready").
   */
  skipPaths?: string[];
  ...
  /**
   * Required certificate types for access (BRC-107/108).
   * If specified, requests must include valid certificates of these types.
   */
  requiredCertificates?: string[];

  /**
   * Trusted certifiers (identity public keys) for certificate verification.
   */
...
export type GatewayConfig = {
  ...
  /** BSV cryptographic identity authentication (BRC-103). */
  bsvAuth?: GatewayBsvAuthConfig;
