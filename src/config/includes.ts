...
class IncludeProcessor {
  ...
  private resolvePath(includePath: string): string {
    ...
    // SECURITY: Confine $include resolution to the config directory to
    // prevent path traversal attacks reading arbitrary files.
    const configDir = path.dirname(this.basePath);
    if (!normalized.startsWith(configDir + path.sep) && normalized !== configDir) {
      throw new ConfigIncludeError(
        `$include path escapes config directory: ${includePath} (resolved: ${normalized}, confined to: ${configDir})`,
        includePath,
      );
    }
