...
const ETC_EDWINPAI_DIR = path.join(ROOT_DIR, "etc", "edwinpai");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "edwinpai.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcEdwinPath(...parts: string[]) {
  return path.join(ETC_EDWINPAI_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path $include outside config directory", () => {
    // SECURITY: $include is confined to the config directory
    const absolute = etcEdwinPath("agents.json");
  ...
  it("rejects parent directory references that escape config directory", () => {
    // SECURITY: $include paths cannot traverse above the config directory
    const files = { [sharedPath("common.json")]: { shared: true } };
    const obj = { $include: "../../shared/common.json" };
    expect(() => resolve(obj, files, configPath("sub", "edwinpai.json"))).toThrow(
      ConfigIncludeError,
    );
    expect(() => resolve(obj, files, configPath("sub", "edwinpai.json"))).toThrow(
