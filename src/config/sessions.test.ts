...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-edwinpai",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-edwinpai#general");
  });
  ...
  it("derives session transcripts dir from EDWINPAI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { EDWINPAI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.EDWINPAI_STATE_DIR = "/custom/state";
    try {
