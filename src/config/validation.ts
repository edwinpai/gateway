...
import type { EdwinPAIConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: EdwinPAIConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: EdwinPAIConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = EdwinPAISchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as EdwinPAIConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as EdwinPAIConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as EdwinPAIConfig)),
    ),
  };
}
