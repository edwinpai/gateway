...
describe("memory index", () => {
  ...
  beforeEach(async () => {
    ...
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-mem-"));
  ...
  it("enforces allowedCollections for builtin fallback reads and search", async () => {
    const cfg = {
      memory: { backend: "builtin" },
      agents: {
        defaults: {
    ...
    await expect(
      result.manager.readFile({
        ...
        accessScope: {
          ...
          actorType: "subagent",
          sessionKey: "agent:main:subagent:test",
          allowedCollections: ["memory-root"],
        },
      }),
    ).resolves.toEqual({ path: "MEMORY.md", text: "Beta knowledge base entry." });

    const scopedResults = await result.manager.search("beta knowledge", {
      maxResults: 10,
      minScore: 0,
      accessScope: {
        agentId: "main",
    ...
    expect(scopedResults.every((entry) => entry.path === "MEMORY.md")).toBe(true);
  });

  it("blocks builtin fallback memory_get direct reads for attach-on-demand profiles", async () => {
    const cfg = {
      memory: { backend: "builtin" },
      agents: {
        defaults: {
