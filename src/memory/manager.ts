/**
 * @deprecated Use QmdMemoryManager (backend: "qmd") instead.
 * The builtin memory backend is retained as a fallback but is no longer the default.
 * qmd provides better search quality (BM25 + OpenAI vector), lower resource usage,
 * and integrates with the workflow-based memory consolidation pipeline.
 */
...
import { resolveMemoryCollections, type ResolvedQmdCollection } from "./backend-config.js";
...
export class MemoryIndexManager implements MemorySearchManager {
  ...
  private readonly collections: ResolvedQmdCollection[];
  ...
  private constructor(params: {
    ...
    this.collections = resolveMemoryCollections({ cfg: params.cfg, agentId: params.agentId });
  ...
  async search(
    ...
    if (!hybrid.enabled) {
      const filteredVectorResults = this.filterResultsByAccessScope(
        vectorResults,
        opts?.accessScope,
      );
      return filteredVectorResults.filter((entry) => entry.score >= minScore).slice(0, maxResults);
    }
    ...
    const filteredMerged = this.filterResultsByAccessScope(merged, opts?.accessScope);
    return filteredMerged.filter((entry) => entry.score >= minScore).slice(0, maxResults);
  }

  private filterResultsByAccessScope<T extends { path: string }>(
    results: T[],
    accessScope?: MemoryAccessScope,
  ): T[] {
    if (!accessScope?.allowedCollections?.length) {
      return results;
    }
    return results.filter((entry) => this.isRelPathAllowed(entry.path, accessScope));
  }

  private isRelPathAllowed(relPath: string, accessScope?: MemoryAccessScope): boolean {
  ...
  async readFile(params: {
    ...
    if (!this.isRelPathAllowed(absPath, params.accessScope)) {
