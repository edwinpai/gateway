...
describe("splitMediaFromOutput", () => {
  ...
  it("accepts trusted absolute paths under /tmp/", () => {
    const result = splitMediaFromOutput("MEDIA:/tmp/tts-rIU1A4/voice-1775202988164.opus");
    expect(result.mediaUrls).toEqual(["/tmp/tts-rIU1A4/voice-1775202988164.opus"]);
    expect(result.text).toBe("");
  });

  it("accepts trusted absolute paths under ~/.edwinpai/media/", () => {
    const { homedir } = require("node:os");
    const home = homedir();
    const mediaPath = `${home}/.edwinpai/media/outbound/test.mp3`;
    const result = splitMediaFromOutput(`MEDIA:${mediaPath}`);
    expect(result.mediaUrls).toEqual([mediaPath]);
    expect(result.text).toBe("");
  });

  it("rejects absolute paths outside trusted prefixes", () => {
    const result = splitMediaFromOutput("MEDIA:/etc/passwd");
    expect(result.mediaUrls).toBeUndefined();
    expect(result.text).toBe("MEDIA:/etc/passwd");
  });

  it("rejects directory traversal within trusted absolute paths", () => {
