...
// Trusted absolute-path prefixes for internally-generated media (TTS, camera, screen, etc.).
// Paths outside these prefixes are rejected to prevent local-file inclusion (LFI).
const TRUSTED_ABSOLUTE_PREFIXES: string[] = ["/tmp/"];

function getTrustedAbsolutePrefixes(): string[] {
  const prefixes = [...TRUSTED_ABSOLUTE_PREFIXES];
  try {
    const home = homedir();
    if (home) {
      prefixes.push(`${home}/.edwinpai/media/`);
    }
  } catch {
    // best-effort; homedir() can throw on some platforms
  }
  return prefixes;
}

function isTrustedAbsolutePath(candidate: string): boolean {
  if (!candidate.startsWith("/")) {
    return false;
  }
  // Reject directory traversal even within trusted prefixes
  if (candidate.includes("..")) {
    return false;
  }
  const prefixes = getTrustedAbsolutePrefixes();
...
function isValidMedia(candidate: string, opts?: { allowSpaces?: boolean }) {
  ...
  // Trusted absolute paths (e.g. /tmp/tts-xxx/voice.opus, ~/.edwinpai/media/...)
  if (isTrustedAbsolutePath(candidate)) {
    return true;
  }
