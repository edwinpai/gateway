...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "EDWINPAI_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.EDWINPAI_STATE_DIR = path.join(home, ".edwinpai");
    ...
    await fs.mkdir(path.join(home, ".edwinpai"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.edwinpai${path.sep}media`);
