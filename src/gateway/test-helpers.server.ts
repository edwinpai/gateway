...
async function setupGatewayTestHome() {
  ...
  previousConfigPath = process.env.EDWINPAI_CONFIG_PATH;
  previousSkipBrowserControl = process.env.EDWINPAI_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.EDWINPAI_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.EDWINPAI_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.EDWINPAI_STATE_DIR = path.join(tempHome, ".edwinpai");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "edwinpai-test-"))
    : path.join(tempHome, ".edwinpai-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.EDWINPAI_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.EDWINPAI_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.EDWINPAI_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.EDWINPAI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.EDWINPAI_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.EDWINPAI_SKIP_CANVAS_HOST;
    } else {
      process.env.EDWINPAI_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.EDWINPAI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.EDWINPAI_GATEWAY_TOKEN = fallbackToken;
  }
