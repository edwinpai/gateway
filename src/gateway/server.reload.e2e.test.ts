...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.EDWINPAI_SKIP_CHANNELS;
    prevSkipGmail = process.env.EDWINPAI_SKIP_GMAIL_WATCHER;
    process.env.EDWINPAI_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.EDWINPAI_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.EDWINPAI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.EDWINPAI_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
