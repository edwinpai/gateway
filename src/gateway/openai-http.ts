...
import { getBsvAuth } from "./bsv-auth.js";
import {
  readJsonBodyOrError,
  sendJson,
...
/**
 * Get identity information from request for logging/tracking
 * Returns BSV identity key if available, otherwise undefined
 */
function _getRequestIdentity(req: IncomingMessage): string | undefined {
  const bsvAuth = getBsvAuth(req);
  return bsvAuth?.identityKey;
}
...
export async function handleOpenAiHttpRequest(
  ...
  const model = typeof payload.model === "string" ? payload.model : "edwinpai";
