...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `edwinpai-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.EDWINPAI_CONFIG_PATH = path.join(root, "edwinpai.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "edwinpai.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "edwinpai-gateway-test"),
        ...fileDefaults,
