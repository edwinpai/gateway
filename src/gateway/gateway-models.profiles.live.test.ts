...
import type { EdwinPAIConfig, ModelProviderConfig } from "../config/types.js";
import { resolveEdwinPAIAgentDir } from "../agents/agent-paths.js";
...
import { ensureEdwinPAIModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.EDWINPAI_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.EDWINPAI_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.EDWINPAI_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<EdwinPAIConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.EDWINPAI_AGENT_DIR,
  ...
  const hostAgentDir = resolveEdwinPAIAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-live-state-"));
  process.env.EDWINPAI_STATE_DIR = tempStateDir;
  ...
  process.env.EDWINPAI_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.edwin-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveEdwinPAIAgentDir();
  const sanitizedCfg: EdwinPAIConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "EdwinPAI live tool probe (local, safe): " +
    ...
    process.env.EDWINPAI_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.EDWINPAI_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureEdwinPAIModelsJson(cfg);
      ...
      const rawModels = process.env.EDWINPAI_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.edwin-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
