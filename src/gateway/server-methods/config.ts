...
import { writeConfigAttestation } from "../../config/config-signature.js";
import {
  CONFIG_PATH,
  loadConfig,
  parseConfigJson5,
...
import { loadEdwinPAIPlugins } from "../../plugins/loader.js";
import {
  ErrorCodes,
  errorShape,
...
export const configHandlers: GatewayRequestHandlers = {
  ...
  "config.schema": ({ params, respond }) => {
    ...
    const pluginRegistry = loadEdwinPAIPlugins({
      config: cfg,
      workspaceDir,
      logger: {
        info: () => {},
  ...
  "config.set": async ({ params, respond, verifiedSignerKey, verifiedEnvelope }) => {
    ...
    if (verifiedSignerKey && verifiedEnvelope) {
      await writeConfigAttestation(CONFIG_PATH, verifiedSignerKey, verifiedEnvelope);
    }
    respond(
      true,
      {
  ...
  "config.patch": async ({ params, respond, verifiedSignerKey, verifiedEnvelope }) => {
  ...
  "config.apply": async ({ params, respond, verifiedSignerKey, verifiedEnvelope }) => {
