...
export function createExecApprovalHandlers(
  ...
  return {
    ...
    // Tool invocation approval resolve (same manager, different event name for desktop compatibility)
    "tool.invoke.approval.resolve": async ({ params, respond, client, context }) => {
      const p = params as { id?: string; decision?: string };
      if (!p.id || !p.decision) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, "requires id and decision"),
        );
        return;
      }
      // Map desktop decisions to exec approval decisions
      ...
      const mapped = decisionMap[p.decision];
      if (!mapped) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid decision"));
        return;
      }
      const resolvedBy = client?.connect?.client?.displayName ?? client?.connect?.client?.id;
      const ok = manager.resolve(p.id, mapped, resolvedBy ?? null);
      if (!ok) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "unknown approval id"));
        return;
      }
      context.broadcast(
        "tool_invoke_approval.resolved",
        { id: p.id, decision: p.decision, resolvedBy, ts: Date.now() },
        { dropIfSlow: true },
      );
      respond(true, { ok: true }, undefined);
    },
