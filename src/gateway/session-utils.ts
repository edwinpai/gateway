...
import { type EdwinPAIConfig, loadConfig } from "../config/config.js";
...
import { canTaskAutoRun, getActiveTask, listTasks } from "./tasks.js";
...
function listConfiguredAgentIds(cfg: EdwinPAIConfig): string[] {
...
export function listAgentsForGateway(cfg: EdwinPAIConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: EdwinPAIConfig): string {
...
function resolveSessionStoreAgentId(cfg: EdwinPAIConfig, canonicalKey: string): string {
...
export function resolveGatewaySessionStoreTarget(params: { cfg: EdwinPAIConfig; key: string }): {
  agentId: string;
  storePath: string;
  canonicalKey: string;
  storeKeys: string[];
...
export function loadCombinedSessionStoreForGateway(cfg: EdwinPAIConfig): {
  storePath: string;
...
export function getSessionDefaults(cfg: EdwinPAIConfig): GatewaySessionsDefaults {
        ...
        activeTask: (() => {
          ...
          return task
            ? {
                ...
                status: task.status,
                criteriaTotal: Array.isArray(task.criteria) ? task.criteria.length : 0,
                criteriaCompleted: Array.isArray(task.completedCriteria)
                  ? task.completedCriteria.length
                  : 0,
                blockedReason: task.blockedReason,
                needsUserReason: task.needsUserReason,
                lastEvaluationReason: task.lastEvaluationReason,
                autoContinueEnabled: task.autoContinueEnabled,
              }
            : undefined;
        })(),
        taskQueue: (() => {
          const tasks = listTasks(entry);
          if (tasks.length === 0) {
            return undefined;
          }
          return {
            total: tasks.length,
            runnable: tasks.filter((task) => canTaskAutoRun(task)).length,
          };
        })(),
      };
    })
