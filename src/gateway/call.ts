...
export function buildGatewayConnectionDetails(
  options: { config?: EdwinPAIConfig; url?: string; configPath?: string } = {},
      ...
      onHelloOk: async () => {
        try {
          ...
          if (SIGNATURE_REQUIRED_METHODS.has(opts.method) && !params?.signedEnvelope) {
            ...
            if (native) {
              const { envelope: signedEnvelope } = await native.signEnvelope({ payload });
              params = { ...(params ?? {}), signedEnvelope };
            } else {
              const privateKey = loadDesktopIdentityKey();
              if (privateKey) {
                const identityCore = createNodeIdentityCoreBinding({
                  async getPublicKey(): Promise<string> {
                    throw new Error(
                      "gateway call signed-envelope transport does not expose getPublicKey()",
                    );
                  },
                  async signHttpRequest(): Promise<never> {
                    throw new Error(
                      "gateway call signed-envelope transport does not expose signHttpRequest()",
                    );
                  },
                  async signEnvelope({ payload: envelopePayload }) {
                    const envelope = createSignedEnvelope(envelopePayload, privateKey);
                    return { payload: envelopePayload, envelope };
                  },
                });
                const { envelope: signedEnvelope } = await identityCore.signEnvelope({ payload });
                params = { ...(params ?? {}), signedEnvelope };
              }
            }
          }
