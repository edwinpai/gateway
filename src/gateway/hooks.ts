...
export function resolveHooksConfig(cfg: EdwinPAIConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-edwinpai-token"] === "string"
      ? req.headers["x-edwinpai-token"].trim()
      : "";
  if (headerToken) {
