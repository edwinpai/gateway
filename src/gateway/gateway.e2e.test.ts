...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.EDWINPAI_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-gw-mock-home-"));
      ...
      process.env.EDWINPAI_SKIP_BROWSER_CONTROL_SERVER = "1";

      const token = `test-${randomUUID()}`;
      process.env.EDWINPAI_GATEWAY_TOKEN = token;

      const workspaceDir = path.join(tempHome, "edwinpai");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.edwin-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".edwinpai");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "edwinpai.json");
      ...
      process.env.EDWINPAI_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.EDWINPAI_GATEWAY_TOKEN = prev.token;
        process.env.EDWINPAI_SKIP_CHANNELS = prev.skipChannels;
        process.env.EDWINPAI_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.EDWINPAI_SKIP_CRON = prev.skipCron;
        process.env.EDWINPAI_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.EDWINPAI_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-wizard-home-"));
    process.env.HOME = tempHome;
