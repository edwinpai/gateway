...
import { getRequestAuthorizer } from "../../bsv-auth.js";
...
import {
  ...
  verifyPermissionTokenProofsRemote,
    ...
    try {
      ...
      if (!client) {
        ...
        if (normalizedClientId) {
          (connectParams.client as { id: string }).id = normalizedClientId;
        }
        ...
        const signedPrompt = connectParams.auth?.signedPrompt;
        const requireSignedPrompt =
          configSnapshot.security?.subscription?.required === true ||
          configSnapshot.security?.requirePermissionTokenProofs === true;
        if (signedPrompt || requireSignedPrompt) {
          if (!signedPrompt) {
            setHandshakeState("failed");
            setCloseCause("signed-prompt-required", {
              client: connectParams.client.id,
              clientDisplayName: connectParams.client.displayName,
              mode: connectParams.client.mode,
              version: connectParams.client.version,
            ...
            send({
              ...
              id: frame.id,
              ok: false,
              error: errorShape(ErrorCodes.INVALID_REQUEST, "signed prompt required"),
            });
            close(1008, "signed prompt required");
            return;
          }

          const authorizer = getRequestAuthorizer({
            maxTimestampAgeMs: configSnapshot.gateway?.bsvAuth?.maxTimestampAge ?? 30000,
          });
          const signedPromptResult = await authorizer.authorizeSignedPrompt(signedPrompt);
          if (!signedPromptResult.authorized) {
            const reason = signedPromptResult.reason ?? "signed prompt invalid";
            setHandshakeState("failed");
            setCloseCause("signed-prompt-invalid", {
              reason,
              client: connectParams.client.id,
              clientDisplayName: connectParams.client.displayName,
              mode: connectParams.client.mode,
          ...
          const tokenScopes = signedPrompt.envelope?.permissionTokens ?? [];

          const subscriptionScope =
            configSnapshot.security?.subscription?.requiredScope ?? "subscription.active";
          if (configSnapshot.security?.subscription?.required === true) {
            const hasSubscription = tokenScopes.some((token) => token.scope === subscriptionScope);
            if (!hasSubscription) {
              const reason = `subscription token missing ()`;
              setHandshakeState("failed");
              setCloseCause("signed-prompt-subscription-missing", {
                reason,
                client: connectParams.client.id,
                clientDisplayName: connectParams.client.displayName,
                mode: connectParams.client.mode,
          ...
          if (tokenScopes.length > 0) {
            const ownership = verifyPermissionTokenOwnership({
              tokens: tokenScopes,
              certHash: signedPrompt.envelope?.certHash,
            });
            if (!ownership.ok) {
              const reason = ownership.reason;
              setHandshakeState("failed");
              setCloseCause("signed-prompt-token-invalid", {
                reason,
                client: connectParams.client.id,
                clientDisplayName: connectParams.client.displayName,
                mode: connectParams.client.mode,
            ...
            if (configSnapshot.security?.requirePermissionTokenProofs === true) {
              const proofResult = verifyPermissionTokenProofs({ tokens: tokenScopes });
              if (!proofResult.ok) {
                const reason = proofResult.reason;
                setHandshakeState("failed");
                setCloseCause("signed-prompt-token-proof-invalid", {
                  reason,
                  client: connectParams.client.id,
                  clientDisplayName: connectParams.client.displayName,
                  mode: connectParams.client.mode,
              ...
              const proofUrl = configSnapshot.security?.permissionTokenProofUrl;
              if (proofUrl) {
                const remoteProof = await verifyPermissionTokenProofsRemote({
                  tokens: tokenScopes,
                  url: proofUrl,
                  timeoutMs: configSnapshot.security?.permissionTokenProofTimeoutMs,
                });
                if (!remoteProof.ok) {
                  const reason = remoteProof.reason;
                  setHandshakeState("failed");
                  setCloseCause("signed-prompt-token-proof-remote-invalid", {
                    reason,
                    client: connectParams.client.id,
                    clientDisplayName: connectParams.client.displayName,
                    mode: connectParams.client.mode,
          ...
          const scopeResolution =
            tokenScopes.length > 0
              ...
              : resolveScopesFromSignedPrompt(signedPrompt, requestedScopes);
          if (!scopeResolution.ok) {
            const reason = scopeResolution.reason;
            setHandshakeState("failed");
            setCloseCause("signed-prompt-scope-mismatch", {
              reason,
              client: connectParams.client.id,
              clientDisplayName: connectParams.client.displayName,
              mode: connectParams.client.mode,
        ...
        const helloOk = {
          ...
          server: {
            version: process.env.EDWINPAI_VERSION ?? process.env.npm_package_version ?? "dev",
