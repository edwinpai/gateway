...
beforeAll(async () => {
  previousToken = process.env.EDWINPAI_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.EDWINPAI_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server sessions", () => {
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-sessions-"));
    ...
    expect(main?.activeTask).toMatchObject({
      goal: "Sweep the desktop UI",
      criteriaTotal: 2,
      criteriaCompleted: 1,
      autoContinueEnabled: true,
    ...
    const taskUpdated = await rpcReq<{
      ok: true;
      key: string;
      activeTask?: { goal?: string; completedCriteria?: string[] };
    }>(ws, "sessions.task.update", {
      key: "agent:main:main",
      taskGoal: "Sweep the desktop UI fully",
      taskCompletedCriteria: ["all reachable views tested", "all fixes verified"],
    });
    expect(taskUpdated.ok).toBe(true);
    expect(taskUpdated.payload?.activeTask).toMatchObject({
      goal: "Sweep the desktop UI fully",
      completedCriteria: ["all reachable views tested", "all fixes verified"],
    });

    const taskFetched = await rpcReq<{
      ok: true;
      key: string;
      activeTask?: { goal?: string; completedCriteria?: string[] };
    }>(ws, "sessions.task.get", {
      key: "agent:main:main",
    });
    expect(taskFetched.ok).toBe(true);
    expect(taskFetched.payload?.activeTask).toMatchObject({
      goal: "Sweep the desktop UI fully",
      completedCriteria: ["all reachable views tested", "all fixes verified"],
    });
  ...
  test("skips stale scheduled task kickoffs after a task is finished", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-sessions-task-kickoff-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
      ...
      entries: {
        main: {
          ...
          tasks: [
            {
              id: "task-a",
              goal: "Fix the tasks loop",
              definitionOfDone: "Completed tasks never get rerun.",
              status: "active",
              criteria: ["root cause found"],
              completedCriteria: [],
              autoContinueEnabled: true,
              active: false,
              delayMs: 200,
    ...
    agentCommand.mockResolvedValue(undefined as never);

    const { ws } = await openClient();

    const selected = await rpcReq<{ ok: true }>(ws, "sessions.tasks.select", {
      key: "agent:main:main",
      taskId: "task-a",
    });
    expect(selected.ok).toBe(true);

    const finished = await rpcReq<{ ok: true; activeTask?: { status?: string } }>(
      ws,
    ...
    expect(finished.payload?.activeTask?.status).toBe("done");
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
