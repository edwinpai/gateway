...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const edwinpai = resolveProfile(resolved, "edwinpai");
    expect(edwinpai?.driver).toBe("edwinpai");
    expect(edwinpai?.cdpPort).toBe(18800);
    expect(edwinpai?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from EDWINPAI_GATEWAY_PORT when unset", () => {
    const prev = process.env.EDWINPAI_GATEWAY_PORT;
    process.env.EDWINPAI_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const edwinpai = resolveProfile(resolved, "edwinpai");
      expect(edwinpai?.cdpPort).toBe(19012);
      expect(edwinpai?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.EDWINPAI_GATEWAY_PORT;
      } else {
        process.env.EDWINPAI_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const edwinpai = resolveProfile(resolved, "edwinpai");
      expect(edwinpai?.cdpPort).toBe(19022);
      expect(edwinpai?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.EDWINPAI_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "edwinpai");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        edwinpai: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("edwinpai");
  });
});
