...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".edwin-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + EdwinPAI orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateEdwinPAIProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_EDWINPAI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_EDWINPAI_BROWSER_COLOR).toUpperCase();
