...
vi.mock("./chrome.js", () => ({
  ...
  launchEdwinPAIChrome: vi.fn(async () => {
  ...
  return {
    ...
    resolved: {
      ...
      profiles: {
        ...
        edwinpai: { cdpPort: 18800, color: "#FF4500" },
      },
    },
    profiles: new Map(),
  };
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const { createBrowserRouteContext } = await import("./server-context.js");
    const state = makeState("edwinpai");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const edwinpai = ctx.forProfile("edwinpai");

    const opened = await edwinpai.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("edwinpai")?.lastTargetId).toBe("CREATED");
  });
});
