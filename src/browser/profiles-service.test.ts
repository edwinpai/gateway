...
vi.mock("./chrome.js", () => ({
  resolveEdwinPAIUserDataDir: vi.fn(() => "/tmp/edwinpai-test/edwinpai/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveEdwinPAIUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "edwinpai-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveEdwinPAIUserDataDir).mockReturnValue(userDataDir);
