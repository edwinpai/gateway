import type { BrowserConfig, BrowserProfileConfig, EdwinPAIConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_EDWINPAI_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_EDWINPAI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "edwinpai" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_EDWINPAI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_EDWINPAI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an EdwinPAI browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for edwinpai/edwinpai CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: EdwinPAIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_EDWINPAI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "edwinpai";
