...
function setWhatsAppDmPolicy(cfg: EdwinPAIConfig, dmPolicy: DmPolicy): EdwinPAIConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: EdwinPAIConfig, allowFrom?: string[]): EdwinPAIConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: EdwinPAIConfig, selfChatMode: boolean): EdwinPAIConfig {
...
async function detectWhatsAppLinked(cfg: EdwinPAIConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so EdwinPAI can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for EdwinPAI" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("edwinpai channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
