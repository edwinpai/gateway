...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.EDWINPAI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*edwinpai-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "edwinpai";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "edwinpai");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "edwinpai-gw");
  ...
  records.push(`_edwin-gw._tcp IN PTR ${instanceLabel}._edwin-gw._tcp`);
  records.push(`${instanceLabel}._edwin-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._edwin-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; edwinpai-content-hash: ${contentHash}\n${contentBody}`;
}
