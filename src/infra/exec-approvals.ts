...
export type SubagentSpawnPolicy = "allow" | "deny" | "ask";
...
export type ExecApprovalsDefaults = {
  ...
  subagentSpawn?: SubagentSpawnPolicy;
};
...
export type ExecApprovalsAgent = ExecApprovalsDefaults & {
  ...
  subagentSpawnAllowAgents?: string[];
};
...
const DEFAULT_SOCKET = "~/.edwinpai/exec-approvals.sock";
const DEFAULT_FILE = "~/.edwinpai/exec-approvals.json";
...
// ---------------------------------------------------------------------------
// Subagent spawn policy helpers
// ---------------------------------------------------------------------------

export function resolveSubagentSpawnPolicy(
  agentId?: string,
  targetAgentId?: string,
): { policy: SubagentSpawnPolicy; preApproved: boolean } {
  ...
  const policy: SubagentSpawnPolicy =
    agent.subagentSpawn ?? wildcard.subagentSpawn ?? defaults.subagentSpawn ?? "ask";

  if (policy === "deny") {
    return { policy, preApproved: false };
  }
  if (policy === "allow") {
    // Check scoped agent filter if present
    const allowAgents = agent.subagentSpawnAllowAgents ?? wildcard.subagentSpawnAllowAgents;
    if (allowAgents && allowAgents.length > 0 && targetAgentId) {
      const normalizedTarget = targetAgentId.toLowerCase();
      const allowed =
        allowAgents.includes("*") || allowAgents.some((a) => a.toLowerCase() === normalizedTarget);
      return { policy, preApproved: allowed };
    }
    return { policy, preApproved: true };
  }
  // "ask" — not pre-approved, needs interactive approval
  return { policy, preApproved: false };
}

export function recordSubagentSpawnAlwaysAllow(agentId?: string, targetAgentId?: string) {
  const file = loadExecApprovals();
  const agentKey = agentId ?? DEFAULT_AGENT_ID;
  const agents = file.agents ?? {};
  const existing = agents[agentKey] ?? {};

  // Set the policy to "allow"
  existing.subagentSpawn = "allow";

  // If a specific target agent was approved, add it to the scoped list
  if (targetAgentId) {
    const allowAgents = Array.isArray(existing.subagentSpawnAllowAgents)
      ? [...existing.subagentSpawnAllowAgents]
      : [];
    const normalizedTarget = targetAgentId.toLowerCase();
    if (!allowAgents.some((a) => a.toLowerCase() === normalizedTarget)) {
      allowAgents.push(targetAgentId);
    }
    existing.subagentSpawnAllowAgents = allowAgents;
  }

  agents[agentKey] = existing;
  file.agents = agents;
