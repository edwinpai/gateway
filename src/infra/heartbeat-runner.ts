...
function resolveActiveHoursTimezone(cfg: EdwinPAIConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: EdwinPAIConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: EdwinPAIConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: EdwinPAIConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: EdwinPAIConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: EdwinPAIConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: EdwinPAIConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export async function runHeartbeatOnce(opts: {
  ...
  if (!heartbeatsEnabled || process.env.EDWINPAI_SKIP_HEARTBEAT === "1") {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: EdwinPAIConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
