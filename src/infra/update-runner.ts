...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "edwinpai doctor",
        managerScriptArgs(manager, "edwinpai", ["doctor", "--non-interactive"]),
        gitRoot,
        { EDWINPAI_UPDATE_IN_PROGRESS: "1" },
      ),
    );
