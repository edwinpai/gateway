...
export function triggerEdwinPAIRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.EDWINPAI_SYSTEMD_UNIT,
        process.env.EDWINPAI_PROFILE,
      );
  ...
  const label =
    process.env.EDWINPAI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.EDWINPAI_PROFILE);
