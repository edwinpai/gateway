...
import { resolveEdwinPAIPackageRoot, resolveEdwinPAIPackageRootSync } from "./edwinpai-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveEdwinPAIPackageRoot({ argv1: normalized });
  if (!packageRoot) {
    return null;
  }
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveEdwinPAIPackageRootSync({
    argv1,
  ...
  if (argv1Dir) {
    // edwinpai.mjs or dist/<bundle>.js
