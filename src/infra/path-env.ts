...
function candidateBinDirs(opts: EnsureEdwinPAIPathOpts): string[] {
  ...
  // Bundled macOS app: `edwinpai` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "edwinpai");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/edwinpai` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "edwinpai"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `edwinpai` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureEdwinPAICliOnPath(opts: EnsureEdwinPAIPathOpts = {}) {
  if (isTruthyEnvValue(process.env.EDWINPAI_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.EDWINPAI_PATH_BOOTSTRAPPED = "1";
