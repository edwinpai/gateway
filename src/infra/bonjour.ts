...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.EDWINPAI_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "EdwinPAI";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(EdwinPAI\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.EDWINPAI_MDNS_HOSTNAME?.trim() || "edwinpai";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
