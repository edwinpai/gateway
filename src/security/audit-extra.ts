...
import type { EdwinPAIConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: EdwinPAIConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: EdwinPAIConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep EDWINPAI_STATE_DIR on a local-only volume and re-run "${formatCliCommand("edwinpai security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: EdwinPAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer EDWINPAI_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: EdwinPAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: EdwinPAIConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: EdwinPAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: EdwinPAIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: EdwinPAIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: EdwinPAIConfig): boolean {
...
function isBrowserEnabled(cfg: EdwinPAIConfig): boolean {
  try {
...
export async function collectPluginsTrustFindings(params: {
  ...
  if (!allowConfigured) {
    ...
    const slackConfig = (params.cfg.channels?.slack ?? {}) as {
      ...
      commands?: { nativeSkills?: unknown };
    };

    const slackConfigured =
      hasString(slackConfig.botToken) ||
      hasString(slackConfig.appToken) ||
      Boolean(
        slackConfig.accounts &&
        Object.values(slackConfig.accounts).some(
    ...
    const skillCommandsLikelyExposed =
      ...
      (slackConfigured &&
        resolveNativeSkillsEnabled({
          providerId: "slack",
          providerSetting: slackConfig.commands?.nativeSkills,
          globalSetting: params.cfg.commands?.nativeSkills,
        }));
...
function listGroupPolicyOpen(cfg: EdwinPAIConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectExposureMatrixFindings(cfg: EdwinPAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
