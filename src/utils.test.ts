...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "edwinpai-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "edwinpai-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "edwinpai-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "edwinpai-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers .edwinpai when the new dir exists", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "edwinpai-config-dir-"));
    try {
      const newDir = path.join(root, ".edwinpai");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/edwinpai")).toBe(path.resolve(os.homedir(), "edwinpai"));
  });
