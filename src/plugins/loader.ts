...
import { discoverEdwinPAIPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: EdwinPAIPluginDefinition;
  register?: EdwinPAIPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as EdwinPAIPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as EdwinPAIPluginDefinition;
...
export function loadEdwinPAIPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverEdwinPAIPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "edwinpai/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: EdwinPAIPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as EdwinPAIPluginModule;
    } catch (err) {
