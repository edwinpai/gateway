...
describe("channels command", () => {
  ...
  it("adds a default discord account with token", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({ ...baseSnapshot });
    await channelsAddCommand(
      {
        channel: "discord",
        account: "default",
        token: "discord-bot-token-1",
      },
      runtime,
      { hasFlags: true },
    );
    ...
    expect(next.channels?.discord?.token).toBe("discord-bot-token-1");
  });
  ...
  it("surfaces Discord privileged intent issues in channels status output", () => {
    ...
    expect(lines.join("\n")).toMatch(/Run: (?:edwinpai|edwinpai)( --profile isolated)? doctor/);
  });
        ...
        telegram: [
          {
            ...
            enabled: true,
            configured: true,
            probe: { ok: true, bot: { username: "edwin_bot" } },
          },
        ],
      },
    });
    expect(lines.join("\n")).toMatch(/bot:@edwin_bot/);
  });
