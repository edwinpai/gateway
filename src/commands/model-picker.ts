...
function resolveConfiguredModelRaw(cfg: EdwinPAIConfig): string {
...
function resolveConfiguredModelKeys(cfg: EdwinPAIConfig): string[] {
...
export async function promptModelAllowlist(params: {
  ...
  if (catalog.length === 0 && allowedKeys.length === 0) {
    const raw = await params.prompter.text({
      ...
      placeholder: "openai-codex/gpt-5.5, anthropic/claude-opus-4-5",
    });
...
export function applyPrimaryModel(cfg: EdwinPAIConfig, model: string): EdwinPAIConfig {
...
export function applyModelAllowlist(cfg: EdwinPAIConfig, models: string[]): EdwinPAIConfig {
  const defaults = cfg.agents?.defaults;
