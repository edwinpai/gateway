...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "EDWINPAI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("EDWINPAI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv EDWINPAI_GATEWAY_TOKEN");
    expect(message).not.toContain("EDWINPAI_GATEWAY_PASSWORD");
  });
