...
function stripUnknownConfigKeys(config: EdwinPAIConfig): {
  config: EdwinPAIConfig;
  removed: string[];
} {
  const parsed = EdwinPAISchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: EdwinPAIConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".edwinpai");
  const targetPath = path.join(targetDir, "edwinpai.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: EdwinPAIConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("edwinpai doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("edwinpai doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "edwinpai doctor --fix" to remove these keys.');
    }
  }
