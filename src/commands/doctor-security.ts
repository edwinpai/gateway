...
import type { EdwinPAIConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: EdwinPAIConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("edwinpai security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("edwinpai configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("edwinpai config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("edwinpai doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "edwinpai config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("edwinpai config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
