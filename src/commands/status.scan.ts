import type { MemoryProviderStatus } from "../memory/public-diagnostics.js";
...
import { getMemoryDiagnosticsRuntime } from "../memory/public-diagnostics.js";
...
const DEFAULT_MEMORY_PLUGIN_ID = defaultSlotIdForKey("memory");
...
function resolveMemoryPluginStatus(cfg: ReturnType<typeof loadConfig>): MemoryPluginStatus {
  ...
  return { enabled: true, slot: raw || DEFAULT_MEMORY_PLUGIN_ID };
}
  ...
  return await withProgress(
    ...
    async (progress) => {
      ...
      const channels = await buildChannelsTable(cfg, {
        ...
        // Set `EDWINPAI_SHOW_SECRETS=0` to force redaction.
        showSecrets: process.env.EDWINPAI_SHOW_SECRETS?.trim() !== "0",
      });
      progress.tick();
      ...
      const memory = await (async (): Promise<MemoryStatusSnapshot | null> => {
        ...
        if (memoryPlugin.slot !== DEFAULT_MEMORY_PLUGIN_ID) {
          return null;
        }
        const agentId = agentStatus.defaultId ?? "main";
        const { runtime } = await getMemoryDiagnosticsRuntime({ cfg, agentId });
        if (!runtime) {
          return null;
        }
        try {
          await runtime.probeVectorAvailability();
        } catch {}
