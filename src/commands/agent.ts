...
export async function agentCommand(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("edwinpai agents list")}" to see configured agents.`,
      );
    }
  }
  ...
  try {
    ...
    if (sessionStore && sessionKey && (taskState.autoContinueEnabled || autoContinue.enabled)) {
      const currentPrompt = buildTaskContinuationPrompt(currentTaskEntry);
      const isContinuationTurn =
        body === AUTO_CONTINUE_PROMPT ||
        body === currentPrompt ||
        body.startsWith("Continue the active task.\nTASK_");
      const taskEvaluation = evaluateTaskState(currentTaskEntry);
      const detectedStopReason = taskEvaluation.stopReason;
      const entry = sessionStore[sessionKey] ??
        sessionEntry ?? { sessionId, updatedAt: Date.now() };
      ...
      if (taskState.autoContinueEnabled) {
        ...
        const nextTask = nextTaskState({
          state: baseTask,
          stopReason: detectedStopReason,
          evaluationReason: taskEvaluation.reason,
        });
        const updatedEntry = setActiveTask(
        ...
        const advancedEntry =
          nextTask.active === true
            ? updatedEntry
            : advanceToNextRunnableTask(updatedEntry, { excludeTaskId: nextTask.id });
        const selectedTask = getActiveTask(advancedEntry);
        const advancedToNextTask =
          selectedTask?.id !== undefined && selectedTask.id !== nextTask.id;
        nextEntry = { ...advancedEntry, sessionId, updatedAt: Date.now() };
        shouldSchedule = shouldContinueTask(nextEntry);
        delayMs = selectedTask?.delayMs ?? nextTask.delayMs ?? taskState.delayMs;
        logSessionStateChange({
          sessionId,
          sessionKey,
          state: shouldSchedule ? "continuing" : "idle",
          reason: advancedToNextTask
            ? "task_queue_advance"
            : `task_${nextTask.lastStopReason ?? detectedStopReason}`,
        });
      } else {
        ...
        const nextState = nextAutoContinueState({
          state: baseState,
          stopReason: classifyAutoContinueStopReason(getAutoContinueText(payloads)),
        });
        nextEntry = { ...entry, sessionId, updatedAt: Date.now(), autoContinue: nextState };
        shouldSchedule = nextState.active === true && nextState.lastStopReason === "continue";
        delayMs = nextState.delayMs ?? autoContinue.delayMs;
        logSessionStateChange({
          sessionId,
          sessionKey,
          state: shouldSchedule ? "continuing" : "idle",
          reason: `autocontinue_${nextState.lastStopReason ?? "continue"}`,
        });
      }
      ...
      await updateSessionStore(storePath, (store) => {
        ...
        store[sessionKey] = { ...existing, ...nextEntry };
      });
      ...
      if (shouldSchedule) {
        const timer = setTimeout(() => {
          ...
          void agentCommand(
            ...
            runtime.error?.(`Auto-continue failed for ${sessionKey}: ${String(err)}`);
          });
        }, delayMs);
        timer.unref?.();
      }
