  ...
  it("infers SSH target from gateway.remote.url and ssh config", async () => {
    ...
    try {
      ...
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "ws://test-host-1.example.ts.net:18789", token: "rtok" },
        },
      });
      resolveSshConfig.mockResolvedValueOnce({
        user: "testuser",
        host: "test-host-1.example.ts.net",
        port: 2222,
      ...
      expect(call.target).toBe("testuser@test-host-1.example.ts.net:2222");
