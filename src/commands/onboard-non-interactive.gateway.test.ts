...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.EDWINPAI_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.EDWINPAI_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "edwinpai");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.EDWINPAI_STATE_DIR = stateDir;
    process.env.EDWINPAI_CONFIG_PATH = path.join(stateDir, "edwinpai.json");
