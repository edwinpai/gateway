...
const SEARCH_TOOL = "https://docs.edwinpai.com/mcp.SearchEdwin";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.edwinpai.com");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('edwinpai docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.edwinpai.com/");
      runtime.log(`Search: ${formatCliCommand('edwinpai docs "your query"')}`);
    }
    return;
  }
