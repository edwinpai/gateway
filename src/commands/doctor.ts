...
import { noteKnowledgeWarnings } from "./doctor-knowledge.js";
import {
...
function resolveMode(cfg: EdwinPAIConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("EdwinPAI doctor");

  const root = await resolveEdwinPAIPackageRoot({
  ...
  let cfg: EdwinPAIConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("edwinpai configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("edwinpai config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("edwinpai setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("edwinpai doctor --fix")}" to apply changes.`);
  }
