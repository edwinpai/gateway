...
const requireAgentDir = () => {
  const agentDir = process.env.EDWINPAI_AGENT_DIR;
  if (!agentDir) {
    throw new Error("EDWINPAI_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.EDWINPAI_STATE_DIR;
  ...
  afterEach(async () => {
    ...
    delete process.env.EDWINPAI_OAUTH_DIR;
  });

  it("writes auth-profiles.json under EDWINPAI_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-oauth-"));
    process.env.EDWINPAI_STATE_DIR = tempStateDir;
    process.env.EDWINPAI_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.EDWINPAI_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to EDWINPAI_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-minimax-"));
    process.env.EDWINPAI_STATE_DIR = tempStateDir;
    process.env.EDWINPAI_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.EDWINPAI_AGENT_DIR;
