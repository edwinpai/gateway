...
beforeEach(() => {
  ...
  resolveEdwinPAIPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.EDWINPAI_STATE_DIR;
  originalUpdateInProgress = process.env.EDWINPAI_UPDATE_IN_PROGRESS;
  process.env.EDWINPAI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "edwinpai-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.EDWINPAI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveEdwinPAIPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadEdwinPAIPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/edwinpai-root.js", () => ({
  resolveEdwinPAIPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "edwinpai-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.EDWINPAI_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when EDWINPAI_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/edwinpai.json",
      exists: true,
      raw: "{}",
    ...
    process.env.EDWINPAI_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
