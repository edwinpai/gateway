...
export function listAgentEntries(cfg: EdwinPAIConfig): AgentEntry[] {
...
function resolveAgentName(cfg: EdwinPAIConfig, agentId: string) {
...
function resolveAgentModel(cfg: EdwinPAIConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: EdwinPAIConfig): AgentSummary[] {
