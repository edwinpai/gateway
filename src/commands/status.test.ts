...
beforeAll(() => {
  previousProfile = process.env.EDWINPAI_PROFILE;
  process.env.EDWINPAI_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.EDWINPAI_PROFILE = previousProfile;
  }
});
...
vi.mock("../memory/public-diagnostics.js", () => ({
  getMemoryDiagnosticsRuntime: vi.fn(async ({ agentId }: { agentId: string }) => ({
    runtime: {
...
vi.mock("../infra/edwinpai-root.js", () => ({
  resolveEdwinPAIPackageRoot: vi.fn().mockResolvedValue("/tmp/edwinpai"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/edwinpai/pnpm-lock.yaml",
      markerPath: "/tmp/edwinpai/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  it("prints JSON when requested", async () => {
    ...
    expect(payload.memoryPlugin.slot).toBe("shad-context");
    expect(payload.memory.vector.available).toBe(true);
    expect(payload.memory.custom.qmd.missingVectorHashes).toBe(411);
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("EdwinPAI status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("edwinpai status --all") ||
          l.includes("edwinpai --profile isolated status --all") ||
          l.includes("edwinpai status --all") ||
          l.includes("edwinpai --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.EDWINPAI_GATEWAY_TOKEN = "abcd1234";
    try {
