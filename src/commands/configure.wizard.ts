    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from edwinpai.json",
        },
      ],
...
async function promptWebToolsConfig(
  nextConfig: EdwinPAIConfig,
  runtime: RuntimeEnv,
): Promise<EdwinPAIConfig> {
  ...
  note(
    [
      ...
      "Docs: https://docs.edwinpai.com/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "EdwinPAI update wizard" : "EdwinPAI configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: EdwinPAIConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.edwinpai.com/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("edwinpai doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.EDWINPAI_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.EDWINPAI_GATEWAY_PASSWORD,
    });
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.EDWINPAI_GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env.EDWINPAI_GATEWAY_PASSWORD;
        ...
        try {
          ...
          note(
            [
              "Docs:",
              "https://docs.edwinpai.com/gateway/health",
              "https://docs.edwinpai.com/gateway/troubleshooting",
            ].join("\n"),
            "Health check help",
          );
        }
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.EDWINPAI_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.EDWINPAI_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.EDWINPAI_GATEWAY_TOKEN;
