...
export async function statusCommand(
  ...
  const memoryValue = (() => {
    ...
    const qmdCustom = memory.custom?.qmd;
    if (qmdCustom && typeof qmdCustom === "object") {
      const missingVectorHashes = (qmdCustom as { missingVectorHashes?: unknown })
        .missingVectorHashes;
      if (typeof missingVectorHashes === "number" && Number.isFinite(missingVectorHashes)) {
        if (missingVectorHashes > 0) {
          parts.push(warn(`embeddings behind ${missingVectorHashes} hashes`));
        } else {
          parts.push(ok("embeddings current"));
        }
      }
    }
  ...
  runtime.log(theme.heading("EdwinPAI status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("edwinpai security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("edwinpai security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.edwinpai.com/faq");
  runtime.log("Troubleshooting: https://docs.edwinpai.com/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("edwinpai status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("edwinpai logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("edwinpai status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("edwinpai gateway probe")}`);
  }
}
