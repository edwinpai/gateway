...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveEdwinPAIAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("edwinpai models auth setup-token")}\` or \`${formatCliCommand("edwinpai configure")}\`.`
          : `Run \`${formatCliCommand("edwinpai configure")}\` or set an API key env var.`;
