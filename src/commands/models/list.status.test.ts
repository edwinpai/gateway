...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveEdwinPAIAgentDir: vi.fn().mockReturnValue("/tmp/edwinpai-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/edwinpai-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/edwinpai-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveEdwinPAIAgentDir: mocks.resolveEdwinPAIAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveEdwinPAIAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/edwinpai-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/edwinpai-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/edwinpai-agent-custom");
