...
export async function updateConfig(
  mutator: (cfg: EdwinPAIConfig) => EdwinPAIConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: EdwinPAIConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: EdwinPAIConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("edwinpai agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
