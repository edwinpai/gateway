...
export async function onTimer(state: CronServiceState) {
  if (state.running) {
    // A previous tick is still executing; schedule a retry so we don't
    // silently drop this wake-up and stall the timer forever.
    if (!state.timer) {
      const retryMs = 5_000;
      state.timer = setTimeout(() => {
        void onTimer(state).catch((err) => {
          state.deps.log.error({ err: String(err) }, "cron: timer retry tick failed");
        });
      }, retryMs);
      state.timer.unref?.();
    }
...
export async function executeJob(
  ...
  try {
    if (job.sessionTarget === "main") {
      ...
      // SECURITY (EDWIN-2026-303): Prefix cron-injected events so the AI model can
      // distinguish scheduled events from genuine system events or user messages.
      const labeledText = `[cron-event job=${job.id}] ${text}`;
      state.deps.enqueueSystemEvent(labeledText, { agentId: job.agentId });
