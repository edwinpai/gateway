...
export function recomputeNextRuns(state: CronServiceState) {
  ...
  for (const job of state.store.jobs) {
    ...
    // Backfill: if a recurring job has never run and should have by now,
    // schedule it to fire immediately so missed runs aren't silently skipped.
    // But only if the miss is recent — if the gap from creation to now spans
    // many intervals, this is likely state loss, not a genuine first-run miss.
    if (
      ...
      const nextFromCreation = computeNextRunAtMs(job.schedule, job.createdAtMs);
      if (nextFromCreation !== undefined && nextFromCreation <= now) {
        const normalNext = computeJobNextRunAtMs(job, now);
        if (normalNext !== undefined) {
          // Only backfill if the job was created recently (within 2 intervals
          // or 24h, whichever is greater) — otherwise it's likely state loss
          // from a restart, not a genuine first-run miss.
          const intervalMs =
            job.schedule.kind === "every"
              ...
              : normalNext - now > 0
                ? normalNext - now
                : 86400000;
          const maxBackfillWindow = Math.max(intervalMs * 2, 86400000);
          if (now - job.createdAtMs <= maxBackfillWindow) {
            state.deps.log.info(
              { jobId: job.id, name: job.name, missedSince: nextFromCreation },
              "cron: backfilling never-run recurring job — scheduling immediate execution",
            );
            job.state.nextRunAtMs = now;
            continue;
          }
          // Old job with cleared state — skip backfill, use normal schedule
          state.deps.log.info(
            { jobId: job.id, name: job.name },
            "cron: skipping backfill for old job (likely state loss) — using normal schedule",
          );
