...
import { stripAgentCompactDirective } from "../../auto-reply/reply/strip-compact.js";
import {
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const cfgWithAgentDefaults: EdwinPAIConfig = {
    ...params.cfg,
  ...
  // Sanitize payloads before delivery — strip internal directives and narration
  // that should not be visible to end users (matching the normal reply path).
  const payloads = (runResult.payloads ?? []).flatMap((payload) => {
    let text = payload.text;
    if (text) {
      // Strip /compact directives (internal housekeeping)
      text = stripAgentCompactDirective(text);
      // Strip HEARTBEAT_OK tokens
      const hb = stripHeartbeatToken(text, { mode: "message" });
      if (hb.didStrip) {
        text = hb.text;
      }
      // Strip NO_REPLY / silent reply tokens
      if (isSilentReplyText(text, SILENT_REPLY_TOKEN)) {
        text = undefined;
      }
    }
    const hasMedia = (payload.mediaUrls?.length ?? 0) > 0 || Boolean(payload.mediaUrl);
    // Drop payloads that have no text and no media
    if (!text?.trim() && !hasMedia) {
      return [];
    }
    return [{ ...payload, text: text?.trim() || undefined }];
  });
