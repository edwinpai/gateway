...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "edwinpai-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".edwinpai", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<EdwinPAIConfig> = {},
): EdwinPAIConfig {
  const base: EdwinPAIConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "edwinpai"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".edwinpai", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
