...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-cron-"));
  return {
...
describe("CronService", () => {
  ...
  it("runs a one-shot main job and disables it after success when requested", async () => {
    ...
    expect(enqueueSystemEvent).toHaveBeenCalledWith(expect.stringContaining("hello"), {
      agentId: undefined,
    });
  ...
  it("runs an isolated job and posts summary to main", async () => {
    ...
    expect(enqueueSystemEvent).toHaveBeenCalledWith(expect.stringContaining("Cron: done"), {
      agentId: undefined,
    });
    expect(requestHeartbeatNow).toHaveBeenCalled();
    cron.stop();
  ...
  it("posts last output to main even when isolated job errors", async () => {
    ...
    expect(enqueueSystemEvent).toHaveBeenCalledWith(
      expect.stringContaining("Cron (error): last output"),
      {
        agentId: undefined,
      },
    );
