...
function makeMissingToolResult(params: {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: "[edwinpai] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ],
    isError: true,
    timestamp: Date.now(),
...
export function repairToolUseResultPairing(messages: AgentMessage[]): ToolUseRepairReport {
  ...
  // - dropping assistant messages with stopReason "error" or "aborted" and their tool results
  //   (these would be stripped by transform-messages.js later, orphaning their tool results)
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    // Drop errored/aborted assistant messages along with their tool results.
    // transform-messages.js (pi-ai) strips these later, which would orphan tool results
    // and cause "unexpected tool_use_id" API rejections from Anthropic.
    const stopReason = (assistant as { stopReason?: unknown }).stopReason;
    if (stopReason === "error" || stopReason === "aborted") {
      const toolCalls = extractToolCallsFromAssistant(assistant);
      const _erroredToolCallIds = new Set(toolCalls.map((t) => t.id));

      // Skip forward past any tool results belonging to this errored assistant message
      let j = i + 1;
      for (; j < messages.length; j += 1) {
        ...
        if (nextRole === "assistant" || nextRole === "user") {
          break;
        }
        if (nextRole === "toolResult") {
          ...
          if (id) {
            // Mark as seen so duplicates later are also dropped
            seenToolResultIds.add(id);
          }
          droppedOrphanCount += 1;
        }
