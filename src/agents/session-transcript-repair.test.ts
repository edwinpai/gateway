...
describe("sanitizeToolUseResultPairing", () => {
  ...
  it("drops errored assistant messages and their tool results", () => {
    const input = [
      { role: "user", content: "hello" },
      {
        ...
        content: [
          ...
          { type: "toolCall", id: "call_err", name: "Read", arguments: { path: "test.txt" } },
        ],
        stopReason: "error",
      },
      {
  ...
  it("drops aborted assistant messages and their tool results", () => {
    const input = [
      { role: "user", content: "hello" },
      {
        role: "assistant",
        content: [
          { type: "toolCall", id: "call_abort1", name: "exec", arguments: { command: "ls" } },
          { type: "toolCall", id: "call_abort2", name: "Read", arguments: { path: "x" } },
        ],
        stopReason: "aborted",
      },
      {
        role: "toolResult",
        toolCallId: "call_abort1",
        toolName: "exec",
    ...
    const out = sanitizeToolUseResultPairing(input);
    // Should keep only the non-errored assistant and user messages
    expect(out.map((m) => m.role)).toEqual(["user", "assistant"]);
    const assistant = out[1] as Extract<AgentMessage, { role: "assistant" }>;
    expect(
      Array.isArray(assistant.content) &&
        (assistant.content[0] as { type?: string; text?: string })?.text,
