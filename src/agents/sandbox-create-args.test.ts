...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "edwinpai-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "edwinpai-sbx-test",
      cfg,
      scopeKey: "main",
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "edwinpai.sandbox=1",
        "--label",
        "edwinpai.sessionKey=main",
        "--label",
        "edwinpai.createdAtMs=1700000000000",
        "--label",
        "edwinpai.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
