...
export function isCliProvider(provider: string, cfg?: EdwinPAIConfig): boolean {
  const normalized = normalizeProviderId(provider);
  if (normalized === "claude-cli") {
    return true;
  }
  // codex-cli removed — codex models route through OpenAI API (openai-codex provider)
...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      console.warn(
        `[edwinpai] Model "${trimmed}" specified without provider. Falling back to "anthropic/${trimmed}". Please use "anthropic/${trimmed}" in your config.`,
      );
