...
import { assertNotSensitivePath } from "./sandbox-paths.js";
import {
...
/**
 * Wrap any file tool with a sensitive path guard for non-sandboxed mode.
 * Blocks access to ~/.ssh, ~/.gnupg, /etc/shadow, etc.
 * SECURITY: EDWIN-2026-003
 */
function wrapSensitivePathGuard(tool: AnyAgentTool): AnyAgentTool {
  return {
    ...tool,
...
function resolveExecConfig(cfg: EdwinPAIConfig | undefined) {
...
export function createEdwinPAICodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createEdwinPAIReadTool(freshReadTool)];
    }
    ...
    if (tool.name === "write") {
      ...
      // Wrap with param normalization + sensitive path guard (EDWIN-2026-003)
      const writeTool = wrapToolParamNormalization(
        createWriteTool(workspaceRoot),
        CLAUDE_PARAM_GROUPS.write,
      );
      return [wrapSensitivePathGuard(writeTool)];
    }
    if (tool.name === "edit") {
      if (sandboxRoot) {
        return [];
      ...
      return [wrapSensitivePathGuard(editTool)];
    }
    return [tool];
  });
