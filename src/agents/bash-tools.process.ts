...
export function createProcessTool(
  ...
  // SECURITY (EDWIN-2026-200): When a scopeKey is set, strictly filter to matching sessions only.
  // When no scopeKey is set, allow access to unscoped sessions only (not other agents' sessions).
  const isInScope = (session?: { scopeKey?: string } | null) => {
    ...
    // No scopeKey on this tool — only see sessions that also have no scopeKey
