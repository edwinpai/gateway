...
export function createCronTool(opts?: CronToolOptions): AnyAgentTool {
  return {
    label: "Cron",
    name: "cron",
    description: `DEPRECATED: The built-in cron system is disabled by default. Use the workflows extension instead. New reliable scheduled automations, reminders, and heartbeat-style jobs should be implemented as workflows or workflow-backed presets.
Workflows are YAML-based, lightweight, and don't require LLM agent overhead. Define workflows in
~/.edwinpai/workspace/workflows/ and schedule them via system crontab + the workflow runner script.
See extensions/workflows/README.md for details.

This tool still works if EDWINPAI_LEGACY_CRON=1 is set. Legacy actions below:
