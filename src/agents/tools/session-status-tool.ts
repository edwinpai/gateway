...
import { getActiveTask } from "../../gateway/tasks.js";
import {
...
export function createSessionStatusTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const task = getActiveTask(resolved.entry);
      const taskLines = task
        ? [
            `🎯 Task: ${task.goal ?? task.id ?? "active"}`,
            task.definitionOfDone ? `✅ Done when: ${task.definitionOfDone}` : undefined,
            Array.isArray(task.criteria) && task.criteria.length > 0
              ? `📋 Criteria: ${task.criteria.length} total / ${(task.completedCriteria ?? []).length} complete`
              : undefined,
            task.blockedReason ? `⛔ Blocked: ${task.blockedReason}` : undefined,
            task.needsUserReason ? `🧑 Needed: ${task.needsUserReason}` : undefined,
            task.lastEvaluationReason ? `🔁 Eval: ${task.lastEvaluationReason}` : undefined,
          ]
            .filter(Boolean)
            .join("\n")
        : undefined;
      const fullStatusText = taskLines ? `${statusText}\n${taskLines}` : statusText;
