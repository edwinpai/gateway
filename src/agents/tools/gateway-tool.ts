...
/**
 * Keys whose values should be redacted when config is returned to the AI model.
 * These are secret/credential fields that the model never needs to see.
 * SECURITY: EDWIN-2026-313
 */
...
/**
 * Deep-clone an object, replacing any values whose keys match REDACT_KEYS
 * with a redacted placeholder. Also strips the `raw` field from config snapshots
 * (which contains the full plaintext config file).
 */
function redactConfigSnapshot(snapshot: unknown): unknown {
  if (!snapshot || typeof snapshot !== "object") {
    return snapshot;
  }
  if (Array.isArray(snapshot)) {
    return snapshot.map(redactConfigSnapshot);
  }
  const result: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(snapshot as Record<string, unknown>)) {
    // Strip raw config text entirely — AI doesn't need it
    if (key === "raw" && typeof value === "string" && value.length > 100) {
      result[key] = "[REDACTED — use config.patch to modify]";
      continue;
    }
    if (REDACT_KEYS.has(key) && typeof value === "string" && value.length > 0) {
      result[key] = REDACTED_VALUE;
      continue;
    }
    result[key] = redactConfigSnapshot(value);
  }
  return result;
}
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (action === "config.get") {
        ...
        // SECURITY (EDWIN-2026-313): Redact secrets before returning to AI model.
        // The AI needs config structure for config.patch/apply, but never needs raw secret values.
        const redacted = redactConfigSnapshot(result);
