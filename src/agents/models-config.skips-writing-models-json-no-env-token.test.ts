...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "edwinpai-models-" });
}

const MODELS_CONFIG: EdwinPAIConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureEdwinPAIModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureEdwinPAIModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("adds openai-codex gpt-5.5 when an openai-codex profile exists", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureEdwinPAIModelsJson } = await import("./models-config.js");
      const { resolveEdwinPAIAgentDir } = await import("./agent-paths.js");
      ...
      await ensureEdwinPAIModelsJson({});
      ...
      expect(parsed.providers["openai-codex"]?.apiKey).toBe("openai-codex-oauth");
      expect(parsed.providers["openai-codex"]?.baseUrl).toBe("https://chatgpt.com/backend-api");
      expect(parsed.providers["openai-codex"]?.models?.map((model) => model.id)).toContain(
        "gpt-5.5",
      );

      const resolved = resolveModel("openai-codex", "gpt-5.5", agentDir, {});
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      ...
      await ensureEdwinPAIModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveEdwinPAIAgentDir(), "models.json");
      const raw = await fs.readFile(modelPath, "utf8");
      const parsed = JSON.parse(raw) as {
