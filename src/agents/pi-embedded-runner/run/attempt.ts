...
import { resolveEdwinPAIDocsPath } from "../../docs-path.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const docsPath = await resolveEdwinPAIDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // Tests sometimes need a stable streamFn reference for mocking, but production
      // must keep the wrapped session streamFn because it injects resolved auth.
      ...
      try {
        ...
        if (!isProbeSession && hookRunner?.hasHooks("before_agent_start")) {
          try {
        ...
        if (!isProbeSession && hookRunner?.hasHooks("agent_end")) {
          hookRunner
            .runAgentEnd(
              {
                messages: messagesSnapshot,
