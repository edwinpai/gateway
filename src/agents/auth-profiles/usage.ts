...
function computeNextProfileUsageStats(params: {
  ...
  if (params.reason === "billing") {
    ...
    // Format errors (e.g. orphaned tool_result blocks) are repaired by
    // session-transcript-repair on retry. Don't set a cooldown — the
    // underlying issue is in the transcript, not the auth profile.
    // We still track the failure count for diagnostics.
  } else {
