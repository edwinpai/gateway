...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "edwinpai.sandbox=1");
  args.push("--label", `edwinpai.sessionKey=${params.scopeKey}`);
  args.push("--label", `edwinpai.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `edwinpai.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("edwinpai.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`edwinpai sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`edwinpai sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("edwinpai sandbox recreate --all");
}
