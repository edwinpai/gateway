...
export function resolveSandboxDockerConfig(params: {
  ...
  // SECURITY: Block dangerous bind mounts that could enable container escape
  // or host compromise. Reject mounts targeting sensitive host paths.
  const DANGEROUS_MOUNT_TARGETS = [
    "/var/run/docker.sock",
    "/run/docker.sock",
    "/etc",
    "/proc",
  ...
  const binds = rawBinds.filter((bind) => {
    const parts = bind.split(":");
    // bind format: "host:container[:opts]"
    const hostPath = parts[0];
    const containerPath = parts.length > 1 ? parts[1] : parts[0];
    for (const dangerous of DANGEROUS_MOUNT_TARGETS) {
      if (
        hostPath === dangerous ||
        hostPath.startsWith(dangerous + "/") ||
        containerPath === dangerous ||
        containerPath.startsWith(dangerous + "/")
      ) {
        logWarn(
          `sandbox: blocked dangerous bind mount "${bind}" ` +
            `(matches restricted path "${dangerous}"). ` +
            "See upstream GHSA sandbox Docker config injection fix.",
        );
        return false;
      }
    }
