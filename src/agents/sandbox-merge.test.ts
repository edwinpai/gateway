...
describe("sandbox config merges", () => {
  ...
  it("merges sandbox docker binds (global + agent combined)", async () => {
    ...
    const resolved = resolveSandboxDockerConfig({
      scope: "agent",
      globalDocker: {
        binds: ["/opt/data/shared:/shared:ro"],
      },
      agentDocker: {
    ...
    expect(resolved.binds).toEqual(["/opt/data/shared:/shared:ro", "/home/user/source:/source:rw"]);
  });
  ...
  it("ignores agent binds under shared scope", async () => {
    ...
    expect(resolved.binds).toEqual(["/opt/data/shared:/shared:ro"]);
  });
