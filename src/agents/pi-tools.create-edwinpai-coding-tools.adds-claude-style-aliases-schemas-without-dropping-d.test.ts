...
const defaultTools = createEdwinPAICodingTools();

describe("createEdwinPAICodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createEdwinPAICodingTools();
    ...
    try {
      ...
      const contents = "Hello from edwinpai read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "edwinpai-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "edwinpai-workspace"),
    ...
    const tools = createEdwinPAICodingTools({ sandbox });
