...
/**
 * Cleanup suspended EdwinPAI CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: EdwinPAIConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-cli-images-"));
  const paths: string[] = [];
