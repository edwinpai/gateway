...
type ModelsConfig = NonNullable<EdwinPAIConfig["models"]>;
...
const OPENAI_CODEX_OAUTH_PLACEHOLDER = "openai-codex-oauth";
const OPENAI_GPT_55_CONTEXT_WINDOW = 272000;
const OPENAI_GPT_55_MAX_TOKENS = 128000;
const OPENAI_GPT_55_COST = {
  // Temporary catalog bridge until upstream pi-ai ships official GPT-5.5 metadata.
  input: 0,
  output: 0,
  cacheRead: 0,
  cacheWrite: 0,
...
function buildOpenAiGpt55Model(): ModelDefinitionConfig {
  return {
    id: "gpt-5.5",
    name: "GPT-5.5",
    reasoning: true,
    ...
    maxTokens: OPENAI_GPT_55_MAX_TOKENS,
  };
}

function buildOpenAiProvider(apiKey: string): ProviderConfig {
  return {
    baseUrl: OPENAI_BASE_URL,
    api: "openai-responses",
    apiKey,
    models: [buildOpenAiGpt55Model()],
  };
}
...
function buildOpenAiCodexProvider(): ProviderConfig {
  return {
    ...
    apiKey: OPENAI_CODEX_OAUTH_PLACEHOLDER,
    models: [buildOpenAiGpt55Model()],
  };
}
...
export async function resolveImplicitProviders(params: {
  ...
  if (openaiKey) {
    providers.openai = buildOpenAiProvider(openaiKey);
  }
...
export async function resolveImplicitCopilotProvider(params: {
  ...
  // Our Copilot auth lives in EdwinPAI's auth-profiles store instead, so we also
  // write a runtime-only auth.json entry for pi-coding-agent to pick up.
  //
  // This is safe because it's (1) within EdwinPAI's agent dir, (2) contains the
  // GitHub token (not the exchanged Copilot token), and (3) matches existing
  // patterns for OAuth-like providers in pi-coding-agent.
  // Note: we deliberately do not write pi-coding-agent's `auth.json` here.
  // EdwinPAI uses its own auth store and exchanges tokens at runtime.
  // `models list` uses EdwinPAI's auth heuristics for availability.
