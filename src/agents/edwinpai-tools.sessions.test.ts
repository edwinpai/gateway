...
describe("sessions tools", () => {
  it("uses number (not integer) in tool schemas for Gemini compatibility", () => {
    ...
    expect(schemaProp("sessions_spawn", "allowedCollections").type).toBe("array");
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createEdwinPAITools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createEdwinPAITools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
