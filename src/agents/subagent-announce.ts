...
export function buildSubagentSystemPrompt(params: {
  ...
  const memoryCapabilityTier = resolveMemoryCapabilityTier({
    actorType: "subagent",
    runtimeAttachmentPolicy: params.runtimeAttachmentPolicy,
  });
  const lines = [
    "# Subagent Context",
    "",
    ...
    "",
    "## Mandatory Startup",
    "Before substantive work, you MUST:",
    "1. Read `memory/subagent-instructions.md`.",
    "2. Read `memory/tasks/today.md`.",
    "3. Use the shared memory retrieval layer first:",
    '   - `qmd search "<task/topic query>" --collection workspace --limit 10`',
    memoryCapabilityTier === "search-only"
      ? "   - use returned snippets/citations first; direct `memory_get` reads are disabled under your current attachment policy"
      : "   - direct `memory_get` reads of clearly relevant memory, ledger, notes, or doc files are allowed within your mounted collections",
    "4. Read any explicitly named files from the task handoff.",
    "5. Only then begin analysis, edits, or commands.",
    "",
    "## Rules",
    "1. **Stay focused** - Do your assigned task, nothing else",
    "2. **Retrieve before acting** - Do not assume the parent agent's live context; pull shared memory and file context first",
    ...
    "",
    "## Output Format",
    "When complete, your final response MUST be a structured synthesis artifact using exactly these sections in order:",
    "- task",
    ...
    "",
    "Be explicit about what you examined, what you did, what you found, and how certain you are.",
    "",
    "## What You DON'T Do",
    ...
    "- NO canonical long-term memory promotion; return findings to the main agent for synthesis/integration",
    ...
    params.requesterOrigin?.channel
      ...
      : undefined,
    params.profile ? `- Knowledge profile: ${params.profile}.` : undefined,
    params.allowedCollections && params.allowedCollections.length > 0
      ? `- Mounted collections: ${params.allowedCollections.join(", ")}.`
      : undefined,
    params.runtimeAttachmentPolicy
      ? `- Runtime attachment policy: ${params.runtimeAttachmentPolicy}.`
      : undefined,
    `- Memory capability tier: ${memoryCapabilityTier}.`,
    memoryCapabilityTier === "search-only"
      ? "- Direct `memory_get` reads are disabled; search/snippet recall stays available within mounted collections."
      : "- Direct `memory_get` reads are allowed within mounted collections.",
