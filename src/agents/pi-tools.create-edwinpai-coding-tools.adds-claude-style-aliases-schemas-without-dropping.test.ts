...
import { __testing, createEdwinPAICodingTools } from "./pi-tools.js";
...
describe("createEdwinPAICodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createEdwinPAITools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createEdwinPAICodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "edwinpai-outside.txt");
