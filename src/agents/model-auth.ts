...
import { getCredentialFromVault } from "../gateway/credential-vault-singleton.js";
...
export async function resolveApiKeyForProvider(params: {
  ...
  // Try credential vault (desktop) before falling back to env vars.
  // During migration, env vars still work as fallback when desktop is offline.
  const vaultCredId = providerToVaultCredentialId(provider);
  if (vaultCredId) {
    const vaultKey = await getCredentialFromVault({
      credentialId: vaultCredId,
      name: `${provider} API key`,
      purpose: "model API call",
      requester: "agent",
    });
    if (vaultKey) {
      return { apiKey: vaultKey, source: `vault:${vaultCredId}`, mode: "api-key" };
    }
  }
  ...
  if (provider === "openai") {
    ...
    if (hasCodex) {
      throw new Error(
        'No API key found for provider "openai". You are authenticated with OpenAI Codex OAuth. Use openai-codex/gpt-5.5 (ChatGPT OAuth) or set OPENAI_API_KEY for openai/gpt-5.5.',
      );
    }
  }
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("edwinpai agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
...
/** Map a provider name to its credential vault ID. */
function providerToVaultCredentialId(provider: string): string | null {
  const normalized = normalizeProviderId(provider);
