...
import { createEdwinPAICodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("edwinpai-ws-", async (workspaceDir) => {
      await withTempDir("edwinpai-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createEdwinPAICodingTools({ workspaceDir });
    ...
    await withTempDir("edwinpai-ws-", async (workspaceDir) => {
      await withTempDir("edwinpai-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello edwinpai");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("edwinpai-ws-", async (workspaceDir) => {
      await withTempDir("edwinpai-override-", async (overrideDir) => {
        const tools = createEdwinPAICodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("edwinpai-sandbox-", async (sandboxDir) => {
      await withTempDir("edwinpai-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "edwinpai-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "edwinpai-sandbox:bookworm-slim",
            containerPrefix: "edwinpai-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createEdwinPAICodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
