...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of EdwinPAI")).toBe("friends-of-edwinpai");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "testuser", "Friends of EdwinPAI"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-edwinpai" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-edwinpai" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of EdwinPAI"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-edwinpai");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-edwinpai": { slug: "friends-of-edwinpai" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of EdwinPAI"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["edwinpai-dm"],
        channelId: "1",
        channelName: "EdwinPAI DM",
        channelSlug: "edwinpai-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
