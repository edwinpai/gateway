...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.edwinpaiCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.edwinpaiCanvasA2UIAction &&
        typeof window.edwinpaiCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.edwinpaiSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("edwinpai:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android EdwinPAI node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.edwinpaiSendUserAction === "function"
        ? window.edwinpaiSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.EDWINPAI_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.EDWINPAI_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".edwinpai", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>EdwinPAI Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
