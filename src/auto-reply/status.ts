...
type AgentConfig = Partial<NonNullable<NonNullable<EdwinPAIConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.edwinpai/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `⚡ EdwinPAI ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: EdwinPAIConfig): string {
  const lines = ["ℹ️ Help", ""];
