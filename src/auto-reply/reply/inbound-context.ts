...
import { normalizeInboundTextNewlines, sanitizeInboundText } from "./inbound-text.js";
...
export function finalizeInboundContext<T extends Record<string, unknown>>(
  ...
  // SECURITY: Sanitize inbound text (strip control chars, reject null bytes,
  // NFC-normalize) before any further processing.
  const rawBody = typeof normalized.Body === "string" ? normalized.Body : "";
  normalized.Body = normalizeInboundTextNewlines(sanitizeInboundText(rawBody));
