...
import { scheduleGatewaySigusr1Restart, triggerEdwinPAIRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting EdwinPAI in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerEdwinPAIRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting EdwinPAI via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
