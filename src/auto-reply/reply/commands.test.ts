...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-commands-"));
...
function buildParams(commandBody: string, cfg: EdwinPAIConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
