...
import type { EdwinPAIConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: EdwinPAIConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: EdwinPAIConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as EdwinPAIConfig),
...
export function getCommandDetection(_cfg?: EdwinPAIConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: EdwinPAIConfig) {
