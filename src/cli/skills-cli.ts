...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("edwinpai skills list")}\` to see all skills.`
      : "No skills found.";
    return message;
  }
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return `Skill "${skillName}" not found. Run \`${formatCliCommand("edwinpai skills list")}\` to see available skills.`;
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.edwinpai.com/cli/skills")}\n`,
    );
