...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), suffix ? `.edwinpai${suffix}` : ".edwinpai");
}
...
export function applyCliProfileEnv(params: {
  ...
  env.EDWINPAI_PROFILE = profile;

  const stateDir = env.EDWINPAI_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.EDWINPAI_STATE_DIR?.trim()) {
    env.EDWINPAI_STATE_DIR = stateDir;
  }

  if (!env.EDWINPAI_CONFIG_PATH?.trim()) {
    env.EDWINPAI_CONFIG_PATH = path.join(stateDir, "edwinpai.json");
  }

  if (profile === "dev" && !env.EDWINPAI_GATEWAY_PORT?.trim()) {
    env.EDWINPAI_GATEWAY_PORT = "19001";
  }
}
