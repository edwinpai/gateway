...
${formatHelpExamples([
  ['edwinpai agent --to +15555550123 --message "status update"', "Start a new session."],
  ['edwinpai agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'edwinpai agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'edwinpai agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['edwinpai agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'edwinpai agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.edwinpai.com/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.edwinpai.com/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['edwinpai agents set-identity --agent main --name "EdwinPAI" --emoji "⚡"', "Set name + emoji."],
  ["edwinpai agents set-identity --agent main --avatar avatars/edwinpai.png", "Set avatar path."],
  [
    "edwinpai agents set-identity --workspace ~/.edwinpai/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "edwinpai agents set-identity --identity-file ~/.edwinpai/workspace/IDENTITY.md --agent main",
