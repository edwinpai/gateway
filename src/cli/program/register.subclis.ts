...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.EDWINPAI_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.EDWINPAI_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<EdwinPAIConfig> => {
...
const entries: SubCliEntry[] = [
  {
    name: "tool",
    description: "Invoke gateway tools (message, TTS, browser, canvas, cron)",
