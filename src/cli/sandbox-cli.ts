...
const SANDBOX_EXAMPLES = {
  main: [
    ["edwinpai sandbox list", "List all sandbox containers."],
    ["edwinpai sandbox list --browser", "List only browser containers."],
    ["edwinpai sandbox recreate --all", "Recreate all containers."],
    ["edwinpai sandbox recreate --session main", "Recreate a specific session."],
    ["edwinpai sandbox recreate --agent mybot", "Recreate agent containers."],
    ["edwinpai sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["edwinpai sandbox list", "List all sandbox containers."],
    ["edwinpai sandbox list --browser", "List only browser containers."],
    ["edwinpai sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["edwinpai sandbox recreate --all", "Recreate all containers."],
    ["edwinpai sandbox recreate --session main", "Recreate a specific session."],
    ["edwinpai sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["edwinpai sandbox recreate --browser --all", "Recreate only browser containers."],
    ["edwinpai sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["edwinpai sandbox explain", "Show effective sandbox config."],
    ["edwinpai sandbox explain --session agent:main:main", "Explain a specific session."],
    ["edwinpai sandbox explain --agent work", "Explain an agent sandbox."],
    ["edwinpai sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.edwinpai.com/cli/sandbox")}\n`,
    )
    .action(() => {
