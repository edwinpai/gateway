...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("edwinpai doctor")}" (or "${formatCliCommand("edwinpai doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("edwinpai gateway install --force")}\` from the same --profile / EDWINPAI_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (status.gateway) {
    ...
    defaultRuntime.log(`${label("UI:")} ${infoText("Desktop app")}`);
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.EDWINPAI_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("edwinpai gateway install")}`),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const env = (service.command?.environment ?? process.env) as NodeJS.ProcessEnv;
      const unit = resolveGatewaySystemdServiceName(env.EDWINPAI_PROFILE);
      defaultRuntime.error(
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("edwinpai status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.edwinpai.com/troubleshooting`);
}
