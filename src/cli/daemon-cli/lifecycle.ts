...
export async function runDaemonRestart(opts: DaemonLifecycleOptions = {}): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      activeGatewayPid = runtime.pid ?? null;
    } catch {
      activeGatewayPid = null;
    }

    const cleanedBefore = await killConflictingGatewayProcesses({ activeGatewayPid });
    if (!json) {
      for (const proc of cleanedBefore.killed) {
        defaultRuntime.log(`Restart cleanup: terminated pid ${proc.pid} (${proc.command})`);
      }
      for (const proc of cleanedBefore.escalated) {
        defaultRuntime.log(
          `Restart cleanup: escalated to SIGKILL for pid ${proc.pid} (${proc.command})`,
        );
      }
    }
    ...
    let restarted = true;
    let restartedGatewayPid: number | null = null;
    try {
      restarted = await service.isLoaded({ env: process.env });
      const runtime = await service.readRuntime(process.env);
      restartedGatewayPid = runtime.pid ?? null;
    } catch {
      restarted = true;
    }

    const cleanedAfter = await killConflictingGatewayProcesses({
      activeGatewayPid: restartedGatewayPid,
    });
    if (!json) {
      for (const proc of cleanedAfter.killed) {
        defaultRuntime.log(
          `Restart cleanup: terminated post-restart pid ${proc.pid} (${proc.command})`,
        );
      }
      for (const proc of cleanedAfter.escalated) {
        defaultRuntime.log(
          `Restart cleanup: escalated post-restart to SIGKILL for pid ${proc.pid} (${proc.command})`,
        );
      }
    }
