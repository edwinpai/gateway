...
import { listMemoryFiles, normalizeExtraMemoryPaths } from "../memory/public-files.js";
import { getMemorySearchManager, type MemorySearchManagerResult } from "../memory/public-ops.js";
import {
  getMemoryRetrievalRuntime,
  type MemoryRetrievalRuntimeResult,
} from "../memory/public-retrieval.js";
...
type MemoryRetrievalManager = NonNullable<MemoryRetrievalRuntimeResult["runtime"]>;
...
function formatStatusTimestamp(value: unknown): string | null {
...
function formatStatusBool(value: unknown): string | null {
  return typeof value === "boolean" ? (value ? "yes" : "no") : null;
}

function formatStatusNumber(value: unknown): string | null {
  return typeof value === "number" && Number.isFinite(value) ? String(value) : null;
}

function readQmdStatusCustom(status: {
  custom?: Record<string, unknown>;
}): Record<string, unknown> | null {
  const qmd = status.custom?.qmd;
  return qmd && typeof qmd === "object" ? (qmd as Record<string, unknown>) : null;
}
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    const qmdCustom = readQmdStatusCustom(status);
    if (qmdCustom) {
      const collections = formatStatusNumber(qmdCustom.collections);
      const lastUpdateAt = formatStatusTimestamp(qmdCustom.lastUpdateAt);
      const lastEmbedAt = formatStatusTimestamp(qmdCustom.lastEmbedAt);
      const embedPending = formatStatusBool(qmdCustom.embedPending);
      const newTokensSinceLastEmbed = formatStatusNumber(qmdCustom.newTokensSinceLastEmbed);
      const embedTokenThreshold = formatStatusNumber(qmdCustom.embedTokenThreshold);
      const totalContentHashes = formatStatusNumber(qmdCustom.totalContentHashes);
      const vectorizedContentHashes = formatStatusNumber(qmdCustom.vectorizedContentHashes);
      const missingVectorHashes = formatStatusNumber(qmdCustom.missingVectorHashes);
      if (collections) {
        lines.push(`${label("QMD collections")} ${info(collections)}`);
      }
      if (lastUpdateAt) {
        lines.push(`${label("QMD last update")} ${info(lastUpdateAt)}`);
      }
      if (lastEmbedAt) {
        lines.push(`${label("QMD last embed")} ${info(lastEmbedAt)}`);
      }
      if (embedPending) {
        lines.push(`${label("QMD embed pending")} ${info(embedPending)}`);
      }
      if (newTokensSinceLastEmbed && embedTokenThreshold) {
        lines.push(
          `${label("QMD embed tokens")} ${info(`${newTokensSinceLastEmbed}/${embedTokenThreshold}`)}`,
        );
      }
      if (missingVectorHashes && Number(missingVectorHashes) > 0) {
        lines.push(
          `${label("QMD embed backlog")} ${warn(`${missingVectorHashes} hashes need vectors`)}`,
        );
      } else if (totalContentHashes && vectorizedContentHashes) {
        lines.push(
          `${label("QMD vector coverage")} ${info(`${vectorizedContentHashes}/${totalContentHashes}`)}`,
        );
      }
    }
...
export function registerMemoryCli(program: Command) {
  const memory = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.edwinpai.com/cli/memory")}\n`,
    );
  ...
  memory
    ...
    .action(
      async (
        ...
        await withManager<MemoryRetrievalManager>({
          getManager: async () => {
            const { runtime, error } = await getMemoryRetrievalRuntime({ cfg, agentId });
            return { manager: runtime, error };
          },
          onMissing: (error) => defaultRuntime.log(error ?? "Memory search disabled."),
          onCloseError: (err) =>
            defaultRuntime.error(`Memory manager close failed: ${formatErrorMessage(err)}`),
          ...
          run: async (runtime) => {
            let results: Awaited<ReturnType<typeof runtime.search>>;
            try {
              results = await runtime.search(query, {
