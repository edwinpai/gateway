...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+(?:edwinpai|edwinpai)\b|^(?:edwinpai|edwinpai)\b/;
...
export function formatCliCommand(
  ...
  const profile = normalizeProfileName(env.EDWINPAI_PROFILE ?? env.EDWIN_PROFILE);
  if (!profile) {
