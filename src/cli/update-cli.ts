...
import { resolveEdwinPAIPackageRoot } from "../infra/edwinpai-root.js";
...
const UPDATE_QUIPS = [
  ...
  "Fresh code, same EdwinPAI. Miss me?",
  ...
  "EdwinPAI has evolved. Faster responses, sharper wit.",
...
const DEFAULT_PACKAGE_NAME = "edwinpai";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const EDWINPAI_REPO_URL = "https://github.com/edwinpai/edwinpai.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".edwinpai");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("edwinpai@")) {
    return trimmed.slice("edwinpai@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "edwinpai.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("edwinpai completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.EDWINPAI_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", EDWINPAI_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `EDWINPAI_GIT_DIR points at a non-git directory: ${params.dir}. Set EDWINPAI_GIT_DIR to an empty folder or an edwinpai checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`EDWINPAI_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveEdwinPAIPackageRoot({
  ...
  defaultRuntime.log(theme.heading("EdwinPAI update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating EdwinPAI..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this EdwinPAI install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("edwinpai doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("edwinpai gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g edwinpai@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g edwinpai@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.EDWINPAI_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.EDWINPAI_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("edwinpai gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("edwinpai doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("edwinpai gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("edwinpai gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `edwinpai update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `EDWINPAI_GIT_DIR points at a non-git directory: ${gitDir}. Set EDWINPAI_GIT_DIR to an empty folder or an edwinpai checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via EDWINPAI_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update EdwinPAI to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["edwinpai update", "Update a source checkout (git)"],
        ["edwinpai update --channel beta", "Switch to beta channel (git + npm)"],
        ["edwinpai update --channel dev", "Switch to dev channel (git + npm)"],
        ["edwinpai update --tag beta", "One-off update to a dist-tag or version"],
        ["edwinpai update --no-restart", "Update without restarting the service"],
        ["edwinpai update --json", "Output result as JSON"],
        ["edwinpai update --yes", "Non-interactive (accept downgrade prompts)"],
        ["edwinpai update wizard", "Interactive update wizard"],
        ["edwinpai --update", "Shorthand for edwinpai update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run edwinpai update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.edwinpai.com/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["edwinpai update status", "Show channel + version status."],
          ["edwinpai update status --json", "JSON output."],
          ["edwinpai update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.edwinpai.com/cli/update")}`,
    )
    .action(async (opts) => {
      try {
