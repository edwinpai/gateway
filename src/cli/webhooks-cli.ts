...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.edwinpai.com/cli/webhooks")}\n`,
    );

  const gmail = webhooks.command("gmail").description("Gmail Pub/Sub hooks (via gws)");

  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + EdwinPAI hooks")
    ...
    .option("--hook-url <url>", "EdwinPAI hook URL")
    .option("--hook-token <token>", "EdwinPAI hook token")
    .option("--push-token <token>", "Push token for gws watch serve")
    .option("--bind <host>", "gws watch serve bind host", DEFAULT_GMAIL_SERVE_BIND)
    .option("--port <port>", "gws watch serve port", String(DEFAULT_GMAIL_SERVE_PORT))
    .option("--path <path>", "gws watch serve path", DEFAULT_GMAIL_SERVE_PATH)
  ...
  gmail
    .command("run")
    .description("Run gws gmail +watch + auto-renew loop")
    ...
    .option("--hook-token <token>", "EdwinPAI hook token")
    .option("--push-token <token>", "Push token for gws watch serve")
    .option("--bind <host>", "gws watch serve bind host")
    .option("--port <port>", "gws watch serve port")
    .option("--path <path>", "gws watch serve path")
