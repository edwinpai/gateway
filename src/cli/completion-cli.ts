...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# EdwinPAI Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(edwinpai completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# EdwinPAI Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(edwinpai completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $EDWINPAI_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "edwinpai") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c edwinpai -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'edwinpai gateway' and not 'edwinpai other gateway', we need to check the sequence.
