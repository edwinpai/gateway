...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("edwinpai node install"), formatCliCommand("edwinpai node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("edwinpai node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    EDWINPAI_LOG_PREFIX: baseEnv.EDWINPAI_LOG_PREFIX ?? "node",
