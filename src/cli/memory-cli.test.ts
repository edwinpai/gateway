...
const getMemoryRetrievalRuntime = vi.fn();
const loadConfig = vi.fn(() => ({}));
const resolveDefaultAgentId = vi.fn(() => "main");

vi.mock("../memory/public-ops.js", () => ({
  getMemorySearchManager,
}));

vi.mock("../memory/public-retrieval.js", () => ({
...
afterEach(async () => {
  ...
  getMemoryRetrievalRuntime.mockReset();
...
describe("memory cli", () => {
  ...
  it("prints qmd embed scheduler status when present", async () => {
    const { registerMemoryCli } = await import("./memory-cli.js");
    const { defaultRuntime } = await import("../runtime.js");
    const close = vi.fn(async () => {});
    getMemorySearchManager.mockResolvedValueOnce({
        ...
        status: () => ({
          ...
          custom: {
            qmd: {
              ...
              totalContentHashes: 5000,
              vectorizedContentHashes: 4589,
    ...
    expect(log).toHaveBeenCalledWith(expect.stringContaining("QMD collections: 2"));
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("QMD last update: 2023-11-14T22:13:20.000Z"),
    );
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("QMD last embed: 2023-11-14T22:15:00.000Z"),
    );
    expect(log).toHaveBeenCalledWith(expect.stringContaining("QMD embed pending: yes"));
    expect(log).toHaveBeenCalledWith(expect.stringContaining("QMD embed tokens: 321/5000"));
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("QMD embed backlog: 411 hashes need vectors"),
    );
    expect(close).toHaveBeenCalled();
  });
  ...
  it("logs close failure after search", async () => {
    ...
    getMemoryRetrievalRuntime.mockResolvedValueOnce({
      runtime: {
        search,
        close,
      },
