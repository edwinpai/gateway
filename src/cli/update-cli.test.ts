...
vi.mock("../infra/edwinpai-root.js", () => ({
  resolveEdwinPAIPackageRoot: vi.fn(),
}));
...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveEdwinPAIPackageRoot } = await import("../infra/edwinpai-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveEdwinPAIPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("EdwinPAI update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveEdwinPAIPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "edwinpai-update-wizard-"));
    const previousGitDir = process.env.EDWINPAI_GIT_DIR;
    try {
      setTty(true);
      process.env.EDWINPAI_GIT_DIR = tempDir;
      ...
      process.env.EDWINPAI_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
