...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    EDWINPAI_STATE_DIR: process.env.EDWINPAI_STATE_DIR,
    EDWINPAI_CONFIG_PATH: process.env.EDWINPAI_CONFIG_PATH,
    EDWINPAI_GATEWAY_PORT: process.env.EDWINPAI_GATEWAY_PORT,
    EDWINPAI_PROFILE: process.env.EDWINPAI_PROFILE,
  };

  beforeEach(() => {
    process.env.EDWINPAI_STATE_DIR = "/tmp/edwinpai-cli-state";
    process.env.EDWINPAI_CONFIG_PATH = "/tmp/edwinpai-cli-state/edwinpai.json";
    delete process.env.EDWINPAI_GATEWAY_PORT;
    delete process.env.EDWINPAI_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.EDWINPAI_STATE_DIR !== undefined) {
      process.env.EDWINPAI_STATE_DIR = originalEnv.EDWINPAI_STATE_DIR;
    } else {
      delete process.env.EDWINPAI_STATE_DIR;
    }

    if (originalEnv.EDWINPAI_CONFIG_PATH !== undefined) {
      process.env.EDWINPAI_CONFIG_PATH = originalEnv.EDWINPAI_CONFIG_PATH;
    } else {
      delete process.env.EDWINPAI_CONFIG_PATH;
    }

    if (originalEnv.EDWINPAI_GATEWAY_PORT !== undefined) {
      process.env.EDWINPAI_GATEWAY_PORT = originalEnv.EDWINPAI_GATEWAY_PORT;
    } else {
      delete process.env.EDWINPAI_GATEWAY_PORT;
    }

    if (originalEnv.EDWINPAI_PROFILE !== undefined) {
      process.env.EDWINPAI_PROFILE = originalEnv.EDWINPAI_PROFILE;
    } else {
      delete process.env.EDWINPAI_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        EDWINPAI_PROFILE: "dev",
        EDWINPAI_STATE_DIR: "/tmp/edwinpai-daemon-state",
        EDWINPAI_CONFIG_PATH: "/tmp/edwinpai-daemon-state/edwinpai.json",
        EDWINPAI_GATEWAY_PORT: "19001",
      },
