...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "edwinpai-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      EDWINPAI_NO_RESPAWN: "1",
      EDWINPAI_STATE_DIR: stateDir,
      EDWINPAI_SKIP_CHANNELS: "1",
      EDWINPAI_SKIP_GMAIL_WATCHER: "1",
      EDWINPAI_SKIP_CRON: "1",
      EDWINPAI_SKIP_BROWSER_CONTROL_SERVER: "1",
      EDWINPAI_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "edwinpai-entry-bootstrap.mjs");
