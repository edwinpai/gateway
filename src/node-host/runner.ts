...
const execHostEnforced = process.env.EDWINPAI_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.EDWINPAI_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureEdwinPAICliOnPath({ pathEnv: process.env.PATH ?? "" });
