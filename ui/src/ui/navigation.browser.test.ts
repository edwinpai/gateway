...
const originalConnect = EdwinPAIApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("edwinpai-app") as EdwinPAIApp;
...
beforeEach(() => {
  EdwinPAIApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__EDWINPAI_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  EdwinPAIApp.prototype.connect = originalConnect;
  window.__EDWINPAI_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/edwinpai/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/edwinpai");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/edwinpai/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__EDWINPAI_CONTROL_UI_BASE_PATH__ = "/edwinpai";
    const app = mountApp("/edwinpai/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/edwinpai");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/edwinpai/sessions");
  });
