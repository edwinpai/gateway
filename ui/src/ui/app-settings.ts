import type { EdwinPAIApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as EdwinPAIApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as EdwinPAIApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as EdwinPAIApp);
  }
  if (host.tab === "agents") {
    const app = host as unknown as EdwinPAIApp;
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as EdwinPAIApp);
    await loadDevices(host as unknown as EdwinPAIApp);
    await loadConfig(host as unknown as EdwinPAIApp);
    await loadExecApprovals(host as unknown as EdwinPAIApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as EdwinPAIApp);
    await loadConfig(host as unknown as EdwinPAIApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as EdwinPAIApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as EdwinPAIApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__EDWINPAI_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as EdwinPAIApp, false),
    loadPresence(host as unknown as EdwinPAIApp),
    loadSessions(host as unknown as EdwinPAIApp),
    loadCronStatus(host as unknown as EdwinPAIApp),
    loadDebug(host as unknown as EdwinPAIApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as EdwinPAIApp, true),
    loadConfigSchema(host as unknown as EdwinPAIApp),
    loadConfig(host as unknown as EdwinPAIApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as EdwinPAIApp, false),
    loadCronStatus(host as unknown as EdwinPAIApp),
    loadCronJobs(host as unknown as EdwinPAIApp),
  ]);
}
