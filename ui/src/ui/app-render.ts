...
import { EdwinPAIApp } from "./app.ts";
        ...
        <div class="topbar-left">
          ...
          <div class="brand">
            <div class="brand-logo">
              <img src="${logoHref}" alt="EdwinPAI" />
            </div>
            <div class="brand-text">
              <div class="brand-title">EDWIN</div>
          ...
          state.tab === "overview"
            ? renderOverview({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  state.chatMessage = "";
                  (state as unknown as EdwinPAIApp).resetToolStream();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
          ...
          state.tab === "chat"
            ? renderChat({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  (state as unknown as EdwinPAIApp).chatStreamStartedAt = null;
                  state.chatQueue = [];
                  (state as unknown as EdwinPAIApp).resetToolStream();
                  (state as unknown as EdwinPAIApp).resetChatScroll();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
                    lastActiveSessionKey: next,
                ...
                onChatScroll: (event) => (state as unknown as EdwinPAIApp).handleChatScroll(event),
                onDraftChange: (next) => (state.chatMessage = next),
                attachments: state.chatAttachments,
                onAttachmentsChange: (next) => (state.chatAttachments = next),
                onSend: () => (state as unknown as EdwinPAIApp).handleSendChat(),
                canAbort: Boolean(state.chatRunId),
                onAbort: () => void (state as unknown as EdwinPAIApp).handleAbortChat(),
                onQueueRemove: (id) => (state as unknown as EdwinPAIApp).removeQueuedMessage(id),
                onNewSession: () =>
                  (state as unknown as EdwinPAIApp).handleSendChat("/new", { restoreDraft: true }),
                showNewMessages: state.chatNewMessagesBelow,
                onScrollToBottom: () => state.scrollToBottom(),
                // Sidebar props for tool output viewing
                sidebarOpen: (state as unknown as EdwinPAIApp).sidebarOpen,
                sidebarContent: (state as unknown as EdwinPAIApp).sidebarContent,
                sidebarError: (state as unknown as EdwinPAIApp).sidebarError,
                splitRatio: (state as unknown as EdwinPAIApp).splitRatio,
                onOpenSidebar: (content: string) =>
                  (state as unknown as EdwinPAIApp).handleOpenSidebar(content),
                onCloseSidebar: () => (state as unknown as EdwinPAIApp).handleCloseSidebar(),
                onSplitRatioChange: (ratio: number) =>
                  (state as unknown as EdwinPAIApp).handleSplitRatioChange(ratio),
        ...
        ${
          state.tab === "config"
            ? renderConfig({
                ...
                searchQuery: (state as unknown as EdwinPAIApp).configSearchQuery,
                activeSection: (state as unknown as EdwinPAIApp).configActiveSection,
                activeSubsection: (state as unknown as EdwinPAIApp).configActiveSubsection,
                onRawChange: (next) => {
                  state.configRaw = next;
                },
                onFormModeChange: (mode) => (state.configFormMode = mode),
                onFormPatch: (path, value) =>
                  updateConfigFormValue(state as unknown as EdwinPAIApp, path, value),
                onSearchChange: (query) =>
                  ((state as unknown as EdwinPAIApp).configSearchQuery = query),
                onSectionChange: (section) => {
                  (state as unknown as EdwinPAIApp).configActiveSection = section;
                  (state as unknown as EdwinPAIApp).configActiveSubsection = null;
                },
                onSubsectionChange: (section) =>
                  ((state as unknown as EdwinPAIApp).configActiveSubsection = section),
                onReload: () => loadConfig(state as unknown as EdwinPAIApp),
                onSave: () => saveConfig(state as unknown as EdwinPAIApp),
                onApply: () => applyConfig(state as unknown as EdwinPAIApp),
                onUpdate: () => runUpdate(state as unknown as EdwinPAIApp),
              })
            : nothing
        }
          ...
          state.tab === "logs"
            ? renderLogs({
                ...
                onExport: (lines, label) =>
                  (state as unknown as EdwinPAIApp).exportLogs(lines, label),
                onScroll: (event) => (state as unknown as EdwinPAIApp).handleLogsScroll(event),
              })
            : nothing
        }
      </main>
